...
import { highestShareDescription, resultDisplayLabel } from "./result-semantics.mjs";
...
function digestRunStatus(run) {
  if (run.queueStatus === "confirmed_finished") {
    if (run.result && run.result !== "unknown") {
      const winner = run.result === "winner" && run.explicitWinnerVariant
        ? ` ${run.explicitWinnerVariant}`
        : "";
      return `${resultDisplayLabel(run.result)}${winner}`;
    }
    return highestShareDescription(run.highestShareVariant) || "Confirmed finished · result unknown";
  }
