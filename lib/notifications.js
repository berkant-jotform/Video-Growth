...
export async function buildDigest(method = "all", ruleOverride = null, profile = null) {
  ...
  const lines = compactLines([
    ...
    ...textSection("Past due manual checks", sections.pastDue, 8),
    sections.pastDue.length > 8 ? `+ ${sections.pastDue.length - 8} more past due checks in the dashboard` : ""
  ]);
  return {
    summary,
    subject,
          ...
          <table role="presentation" width="680" cellspacing="0" cellpadding="0" style="width:680px;max-width:calc(100vw - 28px);background:#ffffff;border:1px solid #dfe6ec;border-radius:14px;overflow:hidden;">
            ...
            <tr>
              <td style="padding:20px 26px;">
                ...
                ${htmlSection("Past due manual checks", sections.pastDue, "These are not confirmed finished; open Studio to verify.", 8, "#b7791f")}
                ${sections.pastDue.length > 8 ? `<p style="margin:14px 0 0;color:#697783;font-size:13px;font-weight:700;">+ ${sections.pastDue.length - 8} more past due checks in the dashboard.</p>` : ""}
...
function htmlRunCard(run, accent) {
  ...
  return `<div style="border:1px solid #dfe6ec;border-left:4px solid ${accent};border-radius:10px;padding:13px 14px;margin:10px 0;background:#ffffff;">
    ...
    <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
      ${studio}
      <span style="color:#7b8792;font-size:12px;">${escapeHtml(run.videoId || "")}</span>
    </div>
  </div>`;
}
...
function digestRunStatus(run) {
  ...
  if (run.queueStatus === "past_due_check") return "Past due check";
