...
export async function buildHistoryWorkbook(exportData) {
  ...
  addEssentialResultsSheet(workbook, exportData);
...
export async function buildAuditPackage({ exportData, workbookBuffer, workbookName }) {
  ...
  const manifest = {
    ...
    grains: {
      essentialResults: "one clean row per persisted logical test_id",
    ...
    counts: {
      essentialResults: exportData.datasets.tests.length,
    ...
    semantics: {
      ...
      formulaSafety:
        "Workbook text is sanitized, typed values stay typed, and no active formulas are generated.",
      reproducibility:
...
function addEssentialResultsSheet(workbook, exportData) {
  const sheet = workbook.addWorksheet("Essential Results", {
    views: [{ state: "frozen", xSplit: 3, ySplit: 1 }]
  });
  const rows = essentialResultRows(exportData);
  const columns = [
    ["Channel", 22],
    ["Video Title", 46],
    ["Test Type", 14],
  ...
  styleEssentialResults(sheet);
}

function essentialResultRows(exportData) {
  const variantsByTest = groupWorkbookRows(
    exportData.datasets.variants || [],
    "test_id"
  );
  const actionsByTest = groupWorkbookRows(
    (exportData.datasets.actions || []).filter((item) => item.active_for_analysis),
    "test_id"
  );
  const qualityByTest = new Map(
    (exportData.datasets.dataQuality || []).map((item) => [item.test_id, item])
  );

  return (exportData.datasets.tests || []).map((test) => {
    const variants = new Map(
      (variantsByTest.get(test.test_id) || []).map((item) => [item.variant_slot, item])
    );
    const latestAction = [...(actionsByTest.get(test.test_id) || [])]
      .sort((left, right) =>
        String(right.created_at_utc || "").localeCompare(String(left.created_at_utc || ""))
      )[0] || null;
    const winningOption =
      test.result === "winner" ? test.explicit_winner_variant || "" : "";
    const finishDate =
      credibleExcelDate(test.finish_date) ||
      credibleExcelDate(String(test.finish_signal_at_utc || "").slice(0, 10));
    const durationDays = Number.isFinite(test.test_duration_hours)
      ? test.test_duration_hours / 24
      : "";

    return {
      Channel: test.channel || "",
      "Video Title": test.current_youtube_title || test.video_title || "",
      "Test Type": titleCase(test.test_type),
      "Studio Video ID": test.video_id || "",
      "Studio URL": test.video_id
        ? `https://studio.youtube.com/video/${test.video_id}/edit`
        : "",
      "Option A": essentialOption(variants.get("A")),
      "Option B": essentialOption(variants.get("B")),
      "Option C": essentialOption(variants.get("C")),
      Result: essentialResultLabel(test.result),
      "Result Detail": titleCase(test.inconclusive_reason),
      "Winning Option": winningOption,
      "Winning Share": winningOption
        ? numberOrBlankLocal(variants.get(winningOption)?.watch_time_share)
        : "",
      "A Share": numberOrBlankLocal(variants.get("A")?.watch_time_share),
      "B Share": numberOrBlankLocal(variants.get("B")?.watch_time_share),
      "C Share": numberOrBlankLocal(variants.get("C")?.watch_time_share),
      "Test Start": credibleExcelDate(test.start_date),
      "Test Finish": finishDate,
      "Duration Days": durationDays,
      "Result Source": resultEvidenceLabel(test.result_evidence),
      "Review Decision": reviewerActionLabel(
        latestAction?.action || test.operational_decision
      ),
      Reviewer: latestAction?.actor_name || "",
      "Reviewed At": credibleExcelDateTime(latestAction?.created_at_utc),
      "Quality Notes": qualityByTest.get(test.test_id)?.issue_codes || "",
      "Test ID": test.test_id
    };
  });
}

function styleEssentialResults(sheet) {
  const header = sheet.getRow(1);
  header.height = 34;
  header.font = { bold: true, color: { argb: "FFFFFFFF" } };
  header.alignment = { vertical: "middle", wrapText: true };
  for (const cell of rowCells(header)) {
    cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FF151B21" } };
    cell.border = {
      bottom: { style: "medium", color: { argb: "FFF23D5A" } }
    };
  }

  const resultColumn = sheet.getColumn("Result").number;
  const winnerColumn = sheet.getColumn("Winning Option").number;
  const decisionColumn = sheet.getColumn("Review Decision").number;
  const studioColumn = sheet.getColumn("Studio URL").number;
  const shareHeaders = ["Winning Share", "A Share", "B Share", "C Share"];
  for (const headerName of shareHeaders) sheet.getColumn(headerName).numFmt = "0.0%";
  sheet.getColumn("Duration Days").numFmt = "0.00";
  sheet.getColumn("Test Start").numFmt = "yyyy-mm-dd";
  sheet.getColumn("Test Finish").numFmt = "yyyy-mm-dd";
  sheet.getColumn("Reviewed At").numFmt = "yyyy-mm-dd hh:mm";
  ...
  for (let rowNumber = 2; rowNumber <= sheet.rowCount; rowNumber += 1) {
    ...
    for (const columnNumber of [2, 6, 7, 8, 23, 24]) {
      row.getCell(columnNumber).alignment = { vertical: "middle", wrapText: true };
    }
    const studioCell = row.getCell(studioColumn);
    const studioUrl = String(studioCell.value || "");
    if (studioUrl) {
      studioCell.value = { text: "Open Studio", hyperlink: studioUrl };
      studioCell.font = { color: { argb: "FF1468A8" }, underline: true, bold: true };
    }
    styleEssentialStatus(row.getCell(resultColumn), String(row.getCell(resultColumn).value || ""));
    styleEssentialStatus(row.getCell(winnerColumn), String(row.getCell(winnerColumn).value || ""));
    styleEssentialStatus(row.getCell(decisionColumn), String(row.getCell(decisionColumn).value || ""));
  }
}

function styleEssentialStatus(cell, value) {
  const normalized = value.toLowerCase();
  if (!normalized) return;
  if (normalized === "winner" || /^selected [abc]$/.test(normalized)) {
    cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFE6F7ED" } };
    cell.font = { bold: true, color: { argb: "FF16794B" } };
  } else if (
    normalized.includes("performed similarly") ||
...
function addSummarySheet(workbook, exportData) {
  ...
  detail(
    rows,
    "Full logical test population",
    exportData.preview.fullPopulation?.logicalTests ?? ""
  );
  detail(
    rows,
    "Selected population share",
    formatRate(exportData.preview.scopeCoverage?.logicalTests)
  );
  detail(
    rows,
    "Share-bearing tests in scope",
    `${exportData.preview.sharesPresent} of ${exportData.preview.fullPopulation?.sharesPresent ?? 0} (${formatRate(exportData.preview.scopeCoverage?.sharesPresent)})`
  );
  detail(
    rows,
...
function dataDictionaryRows() {
  return [
    dictionary("Essential Results", "Channel", "text", "Canonical YouTube channel name."),
    dictionary("Essential Results", "Video Title", "text", "Current YouTube title when available, otherwise the stored test video title."),
    dictionary("Essential Results", "Test Type", "enum", "Whether the row represents a title or thumbnail test.", "Title|Thumbnail"),
    dictionary("Essential Results", "Studio Video ID", "text", "YouTube video ID used to open the exact Studio video.", "", "One row per logical test_id"),
    dictionary("Essential Results", "Studio URL", "hyperlink", "Direct link to the YouTube Studio video editor."),
    dictionary("Essential Results", "Option A", "text", "Option A title text or stored thumbnail reference."),
    dictionary("Essential Results", "Option B", "text", "Option B title text or stored thumbnail reference."),
    dictionary("Essential Results", "Option C", "text", "Option C title text or stored thumbnail reference when a third option exists."),
    dictionary("Essential Results", "Result", "label", "Human-readable canonical YouTube/app result. It is never inferred from the largest share."),
    dictionary("Essential Results", "Result Detail", "text", "Additional reason when YouTube reported an inconclusive or no-clear result."),
    dictionary("Essential Results", "Winning Option", "enum", "A/B/C only when explicit winner evidence identifies a variant.", "A|B|C|blank"),
    dictionary("Essential Results", "Winning Share", "number", "Watch-time share for the explicit winning option when that share is present."),
    dictionary("Essential Results", "A Share", "number", "Canonical 0-1 watch-time share for option A."),
    dictionary("Essential Results", "B Share", "number", "Canonical 0-1 watch-time share for option B."),
    dictionary("Essential Results", "C Share", "number", "Canonical 0-1 watch-time share for option C."),
    dictionary("Essential Results", "Test Start", "date", "Credible stored test start date."),
    dictionary("Essential Results", "Test Finish", "date", "Credible stored finish date, falling back to the explicit finish-signal date."),
    dictionary("Essential Results", "Duration Days", "number", "Test duration in days derived from the evidence-backed duration in Tests."),
    dictionary("Essential Results", "Result Source", "label", "Evidence source supporting the canonical result."),
    dictionary("Essential Results", "Review Decision", "text", "Latest active reviewer action. This does not redefine the YouTube result."),
    dictionary("Essential Results", "Reviewer", "text", "Reviewer who recorded the latest active decision."),
    dictionary("Essential Results", "Reviewed At", "timestamp", "Timestamp of the latest active reviewer decision."),
    dictionary("Essential Results", "Quality Notes", "text", "Semicolon-delimited quality flags from Data Quality."),
    dictionary("Essential Results", "Test ID", "text", "Persisted logical-test identifier for reconciliation.", "", "Joins to Tests.test_id"),
...
function groupWorkbookRows(rows, key) {
...
function essentialOption(variant) {
  if (!variant) return "";
  return variant.variant_text || variant.thumbnail_ref || "";
}

function essentialResultLabel(value) {
  return {
    winner: "Winner",
...
function resultEvidenceLabel(value) {
  return {
    studio_explicit: "YouTube Studio",
    sheet_explicit: "A/B sheet",
    inferred_legacy: "Legacy record",
    unknown: "Unknown"
  }[value] || titleCase(value);
}

function reviewerActionLabel(value) {
  if (["A", "B", "C"].includes(value)) return `Selected ${value}`;
  if (value === "NO_CLEAR") return "Not enough views / No clear";
  if (value === "KEPT_CURRENT") return "Kept current";
  if (value === "RETEST_LATER") return "Retest later";
...
function credibleExcelDate(value) {
  const text = String(value || "").slice(0, 10);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(text) || text < "2005-02-14") return "";
  const date = new Date(`${text}T00:00:00.000Z`);
  return Number.isNaN(date.getTime()) ? "" : date;
}

function credibleExcelDateTime(value) {
  if (!value) return "";
  const date = new Date(value);
  return Number.isNaN(date.getTime()) ? "" : date;
}

function numberOrBlankLocal(value) {
