...
export async function buildAuditPackage({ exportData, workbookBuffer, workbookName }) {
  ...
  const manifest = {
    ...
    grains: {
      ...
      variants: "one row per configured A/B/C variant",
...
function dataDictionaryRows() {
  return [
    ...
    dictionary("Tests", "share_sum_valid", "boolean", "True only when every configured variant has a numeric share and total is 1.00 +/- 0.01."),
    ...
    dictionary("Variants", "variant_slot", "enum", "Configured option slot.", "A|B|C"),
    ...
    dictionary("Data Quality", "issue_codes", "text", "Semicolon-delimited reproducible quality flags."),
    dictionary("Summary", "quality", "enum", "Computed coverage band: low at 40% or lower, partial above 40% through 60%, good above 60%.", "low|partial|good|unknown"),
