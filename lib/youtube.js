...
export async function findYouTubeVideoCandidates({ title, channel = "", apiKey, limit = 3 } = {}) {
  if (!apiKey || !title) return [];
  const cacheKey = `${normalizeBasic(title)}|${normalizeBasic(channel)}|${limit}`;
  ...
  const items = (payload.items || [])
    .map((item) => {
      ...
      const channelBonus = channel && normalizeBasic(channel) === normalizeBasic(snippet.channelTitle || "") ? 0.12 : 0;
      return {
        videoId,
        title: snippet.title || "",
