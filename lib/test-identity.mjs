...
export function sourceLocationAlias(record = {}) {
  ...
  const externalTabId = String(record.externalTabId || "").trim();
  ...
  if (!spreadsheetId || (!externalTabId && !sheetName) || !rowNumber || !sourceKind) return "";
  if (externalTabId) return `sheet:${sourceKind}:${spreadsheetId}:id:${externalTabId}:${rowNumber}`;
...
export function datedVideoAlias(record = {}) {
  const videoId = String(record.videoId || "").trim();
  const testType = String(record.testType || "").trim();
  const startDate = formatDateOnly(record.startDate);
...
export function dedupeIdentityAliasesForPersistence(rows = []) {
  const byAlias = new Map();
  for (const row of rows) {
    const aliasValue = String(row?.alias_value || "").trim();
    if (!aliasValue) continue;
    const existing = byAlias.get(aliasValue);
    if (!existing) {
      byAlias.set(aliasValue, row);
      continue;
    }
    if (existing.test_id === row.test_id) continue;
    const error = new Error(`Identity alias has conflicting owners: ${aliasValue}.`);
    error.code = "ambiguous_identity_alias_batch";
    error.identityConflict = {
      aliasValue,
      conflictingTestIds: [existing.test_id, row.test_id].filter(Boolean)
    };
    throw error;
  }
...
export function resolvePersistedTestId({
  ...
  const candidates = (existingTests || []).filter((test) => {
    ...
    if (startDate && formatDateOnly(test.startDate) === startDate) return true;
