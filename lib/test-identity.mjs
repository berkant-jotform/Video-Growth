...
export function resolvePersistedTestId({
  ...
  const aliasMatches = Array.from(new Set(
    aliases.map((alias) => aliasToTestId.get(alias)).filter(Boolean)
  ));
  if (aliasMatches.length === 1) {
    return { testId: aliasMatches[0], match: "alias", aliases, ambiguous: false };
  }
  if (aliasMatches.length > 1) {
    return { testId: "", match: "ambiguous_alias", aliases, ambiguous: true };
  }
  ...
  if (candidateIds.length > 1) {
    return { testId: "", match: "ambiguous_existing", aliases, ambiguous: true };
  }
