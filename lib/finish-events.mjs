...
export function isLikelyFinishNotification(rawText) {
  ...
  if (/not enough (?:views|impressions|data|traffic)|no clear|inconclusive/i.test(text)) return true;
...
function titleBasedMatch(event, activeRuns = [], context = {}) {
  ...
  for (const run of activeRuns) {
    ...
    for (const candidate of titleCandidates) {
      ...
      const titleMatches =
        exactTitleMatch || similarity >= (channelIdMatches ? 0.66 : channelMatches ? 0.7 : 0.82);
