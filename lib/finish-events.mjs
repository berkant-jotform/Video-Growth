...
export function normalizeWatcherTab(item) {
  ...
  if (/^https:\/\/studio\.youtube\.com\//i.test(target)) {
    const channelId = target.match(/^https:\/\/studio\.youtube\.com\/channel\/(UC[A-Za-z0-9_-]{10,})(?:[/?#]|$)/i)?.[1] || "";
    return { label: label || channelId || target, url: channelId ? `https://studio.youtube.com/channel/${channelId}` : "" };
  }
...
export function parseStudioNotification(input = {}) {
  ...
  const result = classifyStudioResult(rawText);
  return {
    source: input.source || "studio_bell",
    parserVersion: String(input.parserVersion || input.parser_version || "").trim(),
    notificationId: String(input.notificationId || input.notification_id || "").trim(),
    channelIdentitySource: String(input.channelIdentitySource || "").trim(),
    channelIdentityConfidence: String(input.channelIdentityConfidence || "").trim(),
    sourceTabId: Number(input.sourceTabId || 0),
    rawText,
    url,
    videoId,
    channel,
    ...
    resultSemanticsVersion: result.resultSemanticsVersion,
...
export function queueFinishEventPriority(event = {}) {
  const source = String(event.source || "");
  const result = String(event.result || "");
  const evidence = String(event.resultEvidence || event.result_evidence || "");
  const outcome = String(event.detectedOutcome || event.detected_outcome || "");
  if (
    source !== "metadata" &&
    evidence === "studio_explicit" &&
    ["winner", "performed_same", "inconclusive"].includes(result)
  ) {
    return 0;
  }
  if (
    source !== "metadata" &&
    !["", "unknown", "finished_unknown"].includes(outcome)
  ) {
    return 1;
  }
...
export function choosePreferredQueueFinishEvent(events = []) {
  return [...events].sort((left, right) => {
    const priority = queueFinishEventPriority(left) - queueFinishEventPriority(right);
    if (priority !== 0) return priority;
    const resultTextQuality = studioResultTextQuality(right) - studioResultTextQuality(left);
    if (resultTextQuality !== 0) return resultTextQuality;
    const ageQuality = Number(Boolean(right.notificationAge || right.notification_age)) -
      Number(Boolean(left.notificationAge || left.notification_age));
    if (ageQuality !== 0) return ageQuality;
    return eventTimestamp(right) - eventTimestamp(left);
  })[0] || null;
}

export function studioResultTextQuality(event = {}) {
  const text = String(event.rawText || event.raw_text || "").replace(/\s+/g, " ").trim();
  if (!text) return 0;
  const completeTail = /(?:we updated your video to use the winner|results? with very similar performance|not enough (?:views|impressions)(?: to (?:determine|declare) a winner)?|(?:the )?test completed with no winner)[.!]?$/i;
  if (completeTail.test(text)) return 2;
  if (/we updated your video|results? with very similar performance|not enough (?:views|impressions)|completed with no winner/i.test(text)) {
    return 1;
  }
  return 0;
}
...
function eventTimestamp(event = {}) {
  const value = event.observedAt || event.observed_at || "";
...
export function resolveEventOccurredAt({ observedAt, notificationAge, occurredAt } = {}) {
  if (parseNotificationAge(notificationAge)) {
    const estimated = estimateEventOccurredAt(observedAt, notificationAge);
    if (estimated) return estimated;
  }
  if (occurredAt) return occurredAt;
  return estimateEventOccurredAt(observedAt, notificationAge);
}

export function parseNotificationAge(value) {
  if (value && typeof value === "object" && value.days !== null && value.days !== "" && Number.isFinite(Number(value.days))) {
...
export function isLikelyFinishNotification(rawText) {
  ...
  const canonicalBellResult = /\ba\/b\s+test\s+(?:won|performed well for all|inconclusive)\b/i.test(text);
  if (
  ...
  if (/\ba\/b\s+test\s+completed\b/i.test(text) && !canonicalBellResult) {
    return false;
  }
  if (/^test finished\.\s*ran from .{8,180}? to .{8,180}?\.$/i.test(text)) {
    return true;
  }
  if (canonicalBellResult) {
    return true;
  }
  if (/^not enough (?:views|impressions|data|traffic)(?: to (?:determine|declare) a winner)?[.!]?$/i.test(text)) {
    return true;
  }
  if (hasTestContext && /\bwe updated your video to use the winner\b/i.test(text)) {
    return true;
  }
  if (hasTestContext && /\bresults? with very similar performance\b/i.test(text)) {
    return true;
  }
  if (hasTestContext && /\btest completed with no winner\b/i.test(text)) {
    return true;
  }
...
export function isPromotableStudioFinishEvent(input = {}) {
  const event = parseStudioNotification(input);
  const text = String(event.rawText || "").replace(/\s+/g, " ").trim();
  if (!isLikelyFinishNotification(text)) return false;

  const title = cleanNotificationVideoTitle(
    event.videoTitle || extractNotificationVideoTitle(text)
  );
  ...
  const hasBasicTitle =
    ...
    meaningfulTitleTokens.length >= 2;
  const hasStrongTitle =
    normalizedTitle.length >= 14 &&
    meaningfulTitleTokens.length >= 3;
  const hasKnownChannel = Boolean(
    String(event.channelId || "").trim() ||
    (
      String(event.channel || "").trim() &&
      !/^(?:account|channel|unknown source)$/i.test(String(event.channel || "").trim())
    )
  );
  const youtubeResolved = Boolean(
    input.youtubeResolved ||
    input.youtubeResolution?.resolved ||
    input.payload?.youtubeResolution?.resolved
  );
  const hasResolvedIdentity = Boolean(
    String(event.videoId || "").trim() ||
    youtubeResolved ||
    hasKnownChannel
  );
  const hasCompleteResultTail =
    /:\s*(?:we updated your video to use the winner|results? with very similar performance|the test completed with no winner|not enough (?:views|impressions|data|traffic) to (?:determine|declare) a winner)\b/i.test(
      text
    );
  const hasExplicitStudioStatus =
    /^test finished\.\s*ran from .{8,180}? to .{8,180}?\.$/i.test(text);
  const hasCanonicalBellPrefix =
    /\ba\/b\s+test\s+(?:won|performed well for all|inconclusive)\b/i.test(text);

  if (hasExplicitStudioStatus) return true;
  if (hasCompleteResultTail) return hasBasicTitle;
  return hasCanonicalBellPrefix && hasStrongTitle && hasResolvedIdentity;
}
...
export function extractAccessibleFinishEventsFromScan(lastStudioScan = {}) {
  const observedAt = lastStudioScan.checkedAt || new Date().toISOString();
  const events = [];
  for (const tab of Array.isArray(lastStudioScan.tabs) ? lastStudioScan.tabs : []) {
    const tabUrl = String(tab.tabUrl || tab.pageIdentity?.url || "");
    const channelId = String(tab.channelId || "") || tabUrl.match(/\/channel\/(UC[A-Za-z0-9_-]{10,})/i)?.[1] || "";
    if (!channelId) continue;
    const rawChannel = String(tab.channel || "").trim();
    const channel = /^(?:account|channel)$/i.test(rawChannel) ? "" : rawChannel;
    for (const hint of Array.isArray(tab.pageIdentity?.accountHints) ? tab.pageIdentity.accountHints : []) {
      const rawText = String(hint || "").replace(/^new notification\.\s*/i, "").trim();
      if (!/\ba\/b\s+test\s+(?:won|performed well for all|inconclusive)\b/i.test(rawText)) continue;
      events.push({
        ...
        channel,
        channelId,
        notificationAge: rawText.match(/\b\d+\s+(?:minute|hour|day|week|month)s?\s+ago\b/i)?.[0] || "",
        observedAt
      });
      if (events.length >= 80) return expandConnectorEventInputs(events);
    }
  }
  return expandConnectorEventInputs(events).filter((event) => isLikelyFinishNotification(event.rawText));
}

export function consolidateUnmatchedFinishEvents(events = []) {
  const duplicateIds = [];
  const rejectedIds = [];
  const accepted = [];
  const ranked = inferUnmatchedBatchChannels(events)
    .map((event) => ({
      ...event,
      videoTitle: cleanNotificationVideoTitle(event.videoTitle || "")
    }))
    .sort((left, right) => unmatchedEventQuality(right) - unmatchedEventQuality(left));

  for (const event of ranked) {
    if (!isUsefulUnmatchedEvent(event)) {
      if (event.eventId) rejectedIds.push(event.eventId);
      continue;
    }
    const duplicate = accepted.find((candidate) => sameUnmatchedEvent(candidate, event));
    if (duplicate) {
      if (event.eventId) duplicateIds.push(event.eventId);
      continue;
    }
    accepted.push(event);
  }

  accepted.sort((left, right) => new Date(right.observedAt || 0) - new Date(left.observedAt || 0));
  return { events: accepted, duplicateIds, rejectedIds };
}

function inferUnmatchedBatchChannels(events) {
  const batchChannels = new Map();
  for (const event of events) {
    const key = unmatchedBatchKey(event);
    if (!key || !event.channelId) continue;
    if (!batchChannels.has(key)) batchChannels.set(key, new Set());
    batchChannels.get(key).add(String(event.channelId));
  }
  return events.map((event) => {
    if (event.channelId) return event;
    const channels = batchChannels.get(unmatchedBatchKey(event));
    if (!channels || channels.size !== 1) return event;
    return { ...event, channelId: [...channels][0], channelInferredFromBatch: true };
  });
}

function unmatchedBatchKey(event) {
  const timestamp = new Date(event.observedAt || 0).valueOf();
  if (!Number.isFinite(timestamp) || timestamp <= 0) return "";
  return `${event.source || ""}|${Math.floor(timestamp / 1000)}`;
}

function isUsefulUnmatchedEvent(event) {
  const title = String(event.videoTitle || "").trim();
  if (!title && !event.videoId) return false;
  if (!isPromotableStudioFinishEvent(event)) return false;
  const normalized = normalizeMatchText(title);
  const uiTokens = ["notifications", "analytics", "policy", "ideas", "news", "updates", "earn", "known issues"];
  const uiTokenCount = uiTokens.filter((token) => normalized.includes(token)).length;
  if (uiTokenCount >= 3 || /\ball earn\b|\bknown issues policy\b|\banalytics ideas news\b/i.test(title)) return false;
  return true;
}

function sameUnmatchedEvent(left, right) {
  if (String(left.detectedOutcome || "") !== String(right.detectedOutcome || "")) return false;
  const leftTitle = normalizeMatchText(left.videoTitle);
  const rightTitle = normalizeMatchText(right.videoTitle);
  if (!equivalentNotificationTitles(leftTitle, rightTitle)) return false;
  const leftChannel = unmatchedChannelKey(left);
  const rightChannel = unmatchedChannelKey(right);
  return !(leftChannel && rightChannel && leftChannel !== rightChannel);
}

function unmatchedChannelKey(event) {
  const channelId = String(event.channelId || "").trim();
  if (channelId) return channelId.toLowerCase();
  const channel = canonicalChannelName(event.channel || "");
  return /^(?:unknown source|account|channel)$/i.test(channel) ? "" : normalizeMatchText(channel);
}

function unmatchedEventQuality(event) {
  const channel = String(event.channel || "").trim();
  const rawText = String(event.rawText || "");
  const observedAt = new Date(event.observedAt || 0).valueOf();
  return (
    (event.videoId ? 60 : 0) +
    (event.channelId ? 100 : 0) +
    (channel && !/^(?:unknown source|account|channel)$/i.test(channel) ? 50 : 0) +
    (/we updated your video|results? with very similar performance|not enough (?:views|impressions)|completed with no winner/i.test(rawText) ? 20 : 0) +
    (event.notificationAge ? 5 : 0) +
    Math.min(String(event.videoTitle || "").length, 180) / 1000 +
    (Number.isFinite(observedAt) ? observedAt : 0) / 1e16
  );
}
...
function dedupeExpandedEvents(events = []) {
  ...
  for (const original of events) {
    const rawText = String(original.rawText || original.text || "");
    const event = {
      ...original,
      videoTitle: original.videoTitle || original.video_title || extractNotificationVideoTitle(rawText)
    };
    const title = normalizeMatchText(event.videoTitle);
    const scope = [
      event.videoId || event.video_id || "",
      event.channelId || event.channel_id || normalizeMatchText(event.channel),
      detectNotificationOutcome(rawText)
    ].join("|");
    const duplicateIndex = result.findIndex((candidate) => {
      const candidateScope = [
        candidate.videoId || candidate.video_id || "",
        candidate.channelId || candidate.channel_id || normalizeMatchText(candidate.channel),
        detectNotificationOutcome(candidate.rawText || candidate.text || "")
      ].join("|");
      if (candidateScope !== scope) return false;
      return equivalentNotificationTitles(title, normalizeMatchText(candidate.videoTitle));
    });
    if (duplicateIndex < 0) {
      result.push(event);
    } else if (notificationRichness(event) > notificationRichness(result[duplicateIndex])) {
...
function equivalentNotificationTitles(left, right) {
...
function notificationRichness(event) {
  const text = String(event.rawText || event.text || "");
  const completeTail = /we updated your video|results? with very similar performance|not enough (?:views|impressions)|completed with no winner/i.test(text);
  return (completeTail ? 1000 : 0) + (event.notificationAge || event.notification_age ? 200 : 0) + Math.min(text.length, 500);
}
...
function extractNotificationVideoTitle(rawText) {
  const text = String(rawText || "").replace(/\s+/g, " ").trim();
  const directResult = text.match(
    /\b(?:your\s+)?(?:title|thumbnail|a\/b)\s+test\s+(?:finished|completed|ended)\s+(?:for|on)\s+(.+?)(?::\s*(?:results?|winner|not enough|no winner)\b|$)/i
  );
  if (directResult?.[1]) return cleanNotificationVideoTitle(directResult[1]);
...
export function detectNotificationOutcome(rawText) {
  ...
  const result = classifyStudioResult(text);
  if (result.result === "winner") {
    return result.explicitWinnerVariant
      ? `winner_${result.explicitWinnerVariant.toLowerCase()}`
      : "winner";
  }
  if (result.result === "performed_same") return "performed_same";
  if (result.result === "inconclusive") return "inconclusive";
...
export function matchFinishEventToRun(event, activeRuns = []) {
  ...
  const eventTitle = normalizeMatchText(event.videoTitle || extractNotificationVideoTitle(event.rawText));
  ...
  if (event.videoId) {
    ...
    if (exact) {
      if (eventTitle && !eventTitleExactlyMatchesRun(eventTitle, exact)) {
        const alternative = exactTitleAlternativeMatch(event, activeRuns, exact);
...
function exactTitleAlternativeMatch(event, activeRuns, contaminatedRun) {
  const eventTitle = normalizeMatchText(
    event.videoTitle || extractNotificationVideoTitle(event.rawText || "")
  );
  if (!eventTitle) return null;
  const eventChannelId = normalizeChannelId(event.channelId);
  const eventChannel = normalizeMatchText(event.channel);
  const alternatives = activeRuns.filter((run) => {
    if (!run.videoId || run.videoId === contaminatedRun.videoId) return false;
    const runChannelId = normalizeChannelId(run.youtubeChannelId);
    if (eventChannelId && runChannelId && eventChannelId !== runChannelId) return false;
    const runChannel = normalizeMatchText(run.channel || run.youtubeChannelTitle);
    if (eventChannel && runChannel && !["exact", "alias"].includes(channelMatchRelation(eventChannel, runChannel))) {
      return false;
    }
    return eventTitleExactlyMatchesRun(eventTitle, run);
  });
  if (alternatives.length !== 1) return null;
  const run = alternatives[0];
  return {
    run,
    matchedConfidence: eventChannelId && normalizeChannelId(run.youtubeChannelId) === eventChannelId
      ? "title_after_video_id_conflict:exact_title_channel_id"
      : "title_after_video_id_conflict:exact_title_channel",
    score: 0.995
  };
}
...
function titleBasedMatch(event, activeRuns = [], context = {}) {
  ...
  if (!eventTitle) {
    return {
      run: null,
      matchedConfidence: context.skippedVideoId ? "video_id_title_conflict" : "missing_video_title",
      score: 0
    };
  }
  let best = null;
  for (const run of activeRuns) {
    ...
    for (const candidate of titleCandidates) {
      const comparisonText = eventTitle;
      const similarity = titleSimilarity(comparisonText, candidate);
      const exactTitleMatch =
        eventTitle === candidate ||
        (eventTitle.length >= 12 && (candidate.includes(eventTitle) || eventTitle.includes(candidate)));
      const fuzzyAllowed = hasEnoughTitleTokens(comparisonText) && sharedTitleTokenCount(comparisonText, candidate) >= 3;
      const titleMatches =
        exactTitleMatch || (fuzzyAllowed && similarity >= (channelIdMatches ? 0.72 : channelMatches ? 0.74 : 0.9));
      if (!titleMatches) continue;
      if (channelMismatch && !exactTitleMatch && similarity < 0.92) continue;
      const score = channelMatches
        ? exactTitleMatch
          ? channelIdMatches ? 0.98 : 0.94
          : Math.min(0.93, similarity + (channelIdMatches ? 0.05 : 0.03))
        : channelMismatch
...
export function eventTitleMatchesRun(eventTitle, run) {
  ...
  return candidates.some((candidate) => {
    ...
    if (!hasEnoughTitleTokens(eventTitle) || sharedTitleTokenCount(eventTitle, candidate) < 3) return false;
    return titleSimilarity(eventTitle, candidate) >= 0.74;
  });
}

export function eventTitleExactlyMatchesRun(eventTitle, run) {
...
export function notificationTitleMatchesVideoMetadata(event = {}, metadata = {}) {
  const eventTitle = normalizeMatchText(
    event.videoTitle || extractNotificationVideoTitle(event.rawText || "")
  );
  const metadataTitle = String(metadata.title || "").trim();
  if (!eventTitle || !metadataTitle) return true;
  return eventTitleMatchesRun(eventTitle, {
    videoTitle: metadataTitle,
    currentYoutubeTitle: metadataTitle,
    options: {}
  });
}

export function alignFinishEventToMatchedRun(event = {}, match = {}) {
  const runVideoId = String(match?.run?.videoId || "").trim();
  if (!runVideoId) {
    return match?.run?.sourceKind === "app_registry"
      ? { ...event, videoId: "" }
      : event;
  }
  const eventVideoId = String(event.videoId || "").trim();
  const confidence = String(match.matchedConfidence || "");
  const matchedByVideoId = confidence === "video_id" || confidence.endsWith("_video_id");
  const eventTitle = normalizeMatchText(
    event.videoTitle || extractNotificationVideoTitle(event.rawText || "")
  );
  const titleSupportsRun = eventTitle && eventTitleMatchesRun(eventTitle, match.run);
  const shouldReplaceConflict =
    eventVideoId &&
    eventVideoId !== runVideoId &&
    !matchedByVideoId &&
    (confidence.includes("title") || confidence.includes("app_registry")) &&
    titleSupportsRun;
  if (!eventVideoId || shouldReplaceConflict) {
    return {
      ...event,
      videoId: runVideoId
    };
...
function scoreFinishEventRun(event, run) {
  const eventVideoId = String(event.videoId || "").trim();
  const runVideoId = String(run.videoId || "").trim();
  const normalizedEventChannel = normalizeMatchText(event.channel);
  const eventChannelId = normalizeChannelId(event.channelId);
  const runChannelId = normalizeChannelId(run.youtubeChannelId);
  const runChannel = normalizeMatchText(run.channel || run.youtubeChannelTitle);
  ...
  if (eventVideoId && runVideoId === eventVideoId) {
    return {
      run,
  ...
  if (eventVideoId && runVideoId && eventVideoId !== runVideoId) {
    return {
      run,
      score: 0,
      confidence: "low",
      reason: "YouTube video ID differs"
    };
  }
...
export function finishEventHash(event) {
    ...
    return crypto
      ...
      .update(
        [
          ...
          event.testRunId || "",
          event.videoId || "",
          event.detectedOutcome || detectNotificationOutcome(event.rawText)
        ].join("|"),
        "utf8"
      )
      .digest("hex");
  ...
  if (event.notificationId) {
    const syntheticOccurrenceBucket = /^(?:synthetic|aria|page|text|dom):/i.test(event.notificationId) && parseNotificationAge(event.notificationAge)
      ? String(resolveEventOccurredAt({
          occurredAt: event.occurredAt,
          observedAt: event.observedAt,
          notificationAge: event.notificationAge
        }) || "").slice(0, 10)
      : "";
    return crypto
      .createHash("sha1")
      .update([
        ...
        event.videoId || normalizeMatchText(event.videoTitle),
        syntheticOccurrenceBucket,
        event.detectedOutcome || detectNotificationOutcome(event.rawText)
      ].join("|"), "utf8")
      .digest("hex");
  }
  const occurrenceBucket = String(
  ...
  const normalizedTitle =
    normalizeMatchText(event.videoTitle) || normalizeMatchText(event.rawText);
  const eventIdentity =
    event.videoId ||
    normalizedTitle ||
    event.testRunId ||
