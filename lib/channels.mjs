...
export function canonicalChannelName(value) {
  ...
  if (hasChannelWord(raw, "boards") || hasChannelWord(compact, "boards") || /\bjotform\s+boards?\b/.test(raw)) {
    return "Boards";
  }
  if (raw.includes("pdf editor") || compact.includes("pdf editor") || /\bjotform\s+pdf\b/.test(raw)) {
    return "PDF Editor";
  }
  if (
    hasChannelWord(raw, "workflow") ||
    hasChannelWord(compact, "workflow") ||
    hasChannelWord(raw, "workflows") ||
    hasChannelWord(compact, "workflows")
  ) {
    return "Workflow";
  }
  if (hasChannelWord(raw, "noupe") || hasChannelWord(compact, "noupe")) {
    return "Noupe";
  }
  if (isMainJotformLabel(raw) || isMainJotformLabel(compact)) {
    return "Jotform";
  }
  return stripChannelNoise(original) || original;
}

function isMainJotformLabel(value) {
  const text = normalizeChannel(value);
  if (!text.includes("jotform")) return false;
  if (text === "jotform com") return true;
  const remainder = text
    .replace(/\bjotform\b/g, " ")
    .replace(/\b(?:a\s*b|ab|test|tests|title|titles|thumbnail|thumbnails|channel|videos?|current|sheet)\b/g, " ")
    .replace(/\b20\d{2}\b/g, " ")
    .replace(/[^a-z0-9]+/g, " ")
    .trim();
  return !remainder;
