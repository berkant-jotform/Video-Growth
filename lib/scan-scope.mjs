...
export function matchesPostEnrichmentScanFilters(record, filters) {
  ...
  // If YouTube enrichment is temporarily unavailable, update ambiguous thumbnail
  // rows rather than silently leaving their sheet state stale.
  return Boolean(
    filters.channels?.length &&
      record.sourceKind === "thumbnail" &&
      !String(record.youtubeChannelTitle || "").trim() &&
      !String(record.youtubeChannelId || "").trim()
  );
}
