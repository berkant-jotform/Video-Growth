...
export function pruneChannelFilters(selected = [], available = []) {
  const allowed = new Set((available || []).map((item) => String(item || "").trim()).filter((item) => item && item !== "all"));
  return normalizeChannelFilters(selected).filter((item) => allowed.has(item));
}
