...
export async function getAppConfig() {
  ...
  return {
    ...
    historyExportsEnabled:
      String(settings.HISTORY_EXPORTS_ENABLED.value || "").toLowerCase() === "true",
    excludedSheetTabs: parseExcludedSheetTabs(settings[EXCLUDED_SHEET_TABS_KEY].value),
    sourceTabPolicies: parseSourceTabPolicies(settings[SOURCE_TAB_POLICIES_KEY].value),
...
export function publicConfig(config) {
  return {
    ...
    configured: {
      ...
      historyExports: Boolean(config.historyExportsEnabled),
...
function buildSettings(stored) {
  ...
  return Object.fromEntries(
    keys.map((key) => {
      ...
      if (key === "HISTORY_EXPORTS_ENABLED") {
        return [key, { value: "false", source: "default" }];
      }
      if (key === EXTENSION_RUNTIME_CONFIG_KEY) {
        return [key, { value: defaultExtensionRuntimeConfigJson(), source: "default" }];
      }
      if (key === EXCLUDED_SHEET_TABS_KEY) {
        return [key, { value: "[]", source: "default" }];
      }
      if (key === SOURCE_TAB_POLICIES_KEY) {
        return [key, { value: "[]", source: "default" }];
      }
...
function validateSetting(key, value) {
  if (key === EXCLUDED_SHEET_TABS_KEY) {
    parseExcludedSheetTabs(value);
  }
  if (key === SOURCE_TAB_POLICIES_KEY) {
    parseSourceTabPolicies(value);
  }
  if (key === EXTENSION_RUNTIME_CONFIG_KEY) {
  ...
  if (key === "HISTORY_EXPORTS_ENABLED" && !["true", "false"].includes(value.toLowerCase())) {
    throw new Error("HISTORY_EXPORTS_ENABLED must be true or false.");
  }
