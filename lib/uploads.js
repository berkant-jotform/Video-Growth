...
export async function importThumbnailWorkbook({
  file,
...
export async function importThumbnailWorkbookBuffer({
  ...
  const excluded = new Set(excludedSheetNames.map(normalizeSheetName).filter(Boolean));
  let importedCount = 0;
  const importedSheets = [];
  const skippedSheets = [];
  for (const worksheet of workbook.worksheets) {
    if (excluded.has(normalizeSheetName(worksheet.name))) {
      skippedSheets.push(worksheet.name);
      continue;
    }
    const headerMap = thumbnailHeaderMap(worksheet);
    if (!headerMap.size) continue;
    importedSheets.push(worksheet.name);
  ...
  return { uploadId, importedCount, importedSheets, skippedSheets };
}
...
function thumbnailHeaderMap(worksheet) {
  const map = new Map();
  const limit = Math.max(worksheet.rowCount || 0, 1);
  let nonEmptyRows = 0;
  for (let rowNumber = 1; rowNumber <= limit && map.size < 2 && nonEmptyRows < 200; rowNumber += 1) {
    const candidate = new Map();
    let nonEmpty = false;
    worksheet.getRow(rowNumber).eachCell((cell, colNumber) => {
      if (String(normalizeWorkbookCell(cell.value) ?? "").trim()) nonEmpty = true;
