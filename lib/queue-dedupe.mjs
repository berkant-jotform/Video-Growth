...
export function dedupeQueueRuns(runs = []) {
  ...
  for (const run of runs) {
    ...
    // The logical key already includes video, test type, start date, and
    // options. Different row numbers with that exact identity are duplicate
    // copies, not retests.
...
function applyRetestFlags(runs) {
  ...
  for (const run of runs) {
    ...
    const materialKey = `${formatDateOnly(run.startDate)}|${run.optionFingerprint || ""}`;
...
function hasAuthoritativeRetestEvidence(run) {
  ...
  return Boolean(formatDateOnly(run.startDate) || String(run.optionFingerprint || "").trim());
}
