...
export function buildResultMigrationPlan({
  ...
  for (const cluster of allClusters) {
    ...
    const dataQualityFlag = terminalEvidence
      ? ""
      : formatDateOnly(representative.startDate)
...
function hasTerminalEvidence({ runs, events }) {
  ...
  return runs.some((run) => {
    if (formatDateOnly(run.finishDate)) return true;
...
function representativeScore(run) {
  return (
    (run.videoId ? 100 : 0) +
    (formatDateOnly(run.startDate) ? 50 : 0) +
    (formatDateOnly(run.finishDate) ? 40 : 0) +
