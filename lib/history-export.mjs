...
export function normalizeHistoryExportRequest(input = {}) {
  ...
  const rows = HISTORY_EXPORT_ROWS.includes(input.rows) ? input.rows : "all_completed";
...
function selectTests(source, request) {
  ...
  return (source.tests || []).filter((test) => {
    ...
    if (
      ...
      normalize(canonicalChannelName(test.channel) || test.channel) !==
        normalize(canonicalChannelName(request.filters.channel) || request.filters.channel)
    ) {
      return false;
    }
    if (
...
function testDatasetRow({
  ...
  const startDate = formatDateOnly(test.startDate);
  const finishDate = formatDateOnly(test.finishDate);
  ...
  return {
    ...
    channel: canonicalChannelName(test.channel) || test.channel || "",
...
function sourceRecordDatasetRow(record) {
  return {
    ...
    start_date: formatDateOnly(record.startDate),
    finish_date: formatDateOnly(record.finishDate),
...
function buildPreview({
  ...
  const fullPopulation = populationCounts(source, {
    ...normalized,
    rows: "everything",
    filters: {
      search: "",
  ...
  const rowScopes = Object.fromEntries(
    HISTORY_EXPORT_ROWS.map((rows) => [
      rows,
      populationCounts(source, { ...normalized, rows })
    ])
  );
  const scopeCoverage = {
    logicalTests: ratio(tests.length, fullPopulation.logicalTests),
    sourceRecords: ratio(sourceRecords.length, fullPopulation.sourceRecords),
    sharesPresent: ratio(
      tests.filter((test) => test.share_present).length,
      fullPopulation.sharesPresent
    )
  };
  if (!tests.length) {
  ...
  if (
    tests.length &&
    Number.isFinite(scopeCoverage.logicalTests) &&
    scopeCoverage.logicalTests < 0.25
  ) {
    warnings.push(
      warning(
        "degrading",
        `This scope contains ${tests.length} of ${fullPopulation.logicalTests} logical tests (${formatWholePercent(scopeCoverage.logicalTests)}).`,
        "widen_scope"
      )
    );
  }
...
function populationCounts(source, request) {
  const tests = selectTests(source, request);
  const testIds = new Set(tests.map((test) => test.testId));
  return {
    logicalTests: tests.length,
    sourceRecords: (source.sourceRecords || []).filter((item) =>
      testIds.has(item.testId)
    ).length,
    sharesPresent: tests.filter((test) => shareAnalysis(test).sharePresent).length
  };
}
...
function durationFields({ test, actions, events, generatedAtUtc }) {
  ...
  const openDays = isOpen ? ageDays(formatDateOnly(test.startDate), generatedAtUtc) : null;
  return {
