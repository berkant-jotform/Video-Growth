...
export function computeCoverage({ tests = [], generatedAtUtc = new Date().toISOString() }) {
  ...
  const metrics = {
    ...
    strictShares: (test) =>
      test.test_type === "title" && Boolean(test.share_sum_valid)
  };
...
function variantDatasetRows(test, exportedTest) {
  ...
  return ["A", "B", "C"]
    .filter((slot) => hasConfiguredOption(options[slot]))
    .map((slot) => ({
      test_id: test.testId,
      video_id: test.videoId || "",
      channel: test.channel || "",
...
function shareAnalysis(test) {
  const options = test.options || {};
  const shares = test.watchTimeShare || {};
  const configured = ["A", "B", "C"].filter((slot) => hasConfiguredOption(options[slot]));
  const numeric = configured
    .map((slot) => [slot, numericShare(shares[slot])])
    .filter(([, value]) => Number.isFinite(value));
  const complete = configured.length >= 2 && numeric.length === configured.length;
