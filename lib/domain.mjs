...
export function parseSheetRecords({ spreadsheetId, sourceKind, sheetName, values, today }) {
  ...
  return values.slice(headerRowIndex + 1).flatMap((row, offset) => {
    if (!rowRepresentsTest(row, { startIdx, finishIdx, doneIdx, optionIndices })) {
      return [];
    }
    return [
      buildTestRun({
...
function rowRepresentsTest(row, { startIdx, finishIdx, doneIdx, optionIndices }) {
  ...
  const hasLifecycleSignal = Boolean(
    parseDate(valueAt(row, startIdx)).date ||
    parseDate(valueAt(row, finishIdx)).date ||
