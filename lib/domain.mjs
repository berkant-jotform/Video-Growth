import crypto from "node:crypto";
import { canonicalChannelName } from "./channels.mjs";
import {
  analyzeShares,
  classifySheetResult,
  RESULT_SEMANTICS_VERSION
} from "./result-semantics.mjs";
import { testContentHash } from "./test-identity.mjs";
import { formatDateOnly } from "./date-only.mjs";

export const NO_CLEAR_WINNER_TEXT = "not enough impressions";
const NO_CLEAR_RESULT_RE =
  /not enough (?:views|impressions|data|traffic)|no (?:clear )?winner|similar performance|performed well for all|inconclusive|could(?:\s+not|n't) determine/i;
const VIDEO_ID_RE =
  /(?:youtu\.be\/|youtube\.com\/(?:watch\?[^\s"']*?v=|shorts\/|live\/|embed\/)|studio\.youtube\.com\/video\/)([A-Za-z0-9_-]{6,})/i;

export function extractVideoId(...values) {
  const joined = values.map((value) => String(value || "")).join(" ");
  const match = joined.match(VIDEO_ID_RE);
  return match ? match[1] : "";
}

export function extractSpreadsheetId(value) {
  const text = String(value || "").trim();
  if (!text) return "";
  const match = text.match(/\/spreadsheets\/d\/([a-zA-Z0-9-_]+)/);
  if (match) return match[1];
  return text;
}

export function normalizeHeader(value) {
  return String(value || "")
    .replace(/\n/g, " ")
    .trim()
    .toLowerCase()
    .replace(/\s+/g, " ")
    .replace(/[–—]/g, "-");
}

export function headerIndex(headers, names) {
  const normalized = headers.map(normalizeHeader);
  const wanted = names.map(normalizeHeader);
  const idx = normalized.findIndex((header) => wanted.includes(header));
  return idx >= 0 ? idx : null;
}

export function parseBool(value) {
  return ["true", "yes", "done", "complete", "completed", "closed", "1", "y"].includes(
    String(value || "").trim().toLowerCase()
  );
}

export function parseDate(value, referenceDate = "") {
  if (value instanceof Date && !Number.isNaN(value.valueOf())) {
    return { date: formatDateOnly(value), present: true };
  }

  const text = String(value || "").replace(/\u2009/g, " ").trim();
  if (!text || text === "-") return { date: "", present: false };

  const iso = text.match(/^(\d{4})-(\d{1,2})-(\d{1,2})(?:\s.*)?$/);
  if (iso) return buildDate(iso[1], iso[2], iso[3], true);

  const slash = text.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/);
  if (slash) {
    const first = Number(slash[1]);
    const second = Number(slash[2]);
    if (first > 12) return buildDate(slash[3], slash[2], slash[1], true);
    if (second > 12) return buildDate(slash[3], slash[1], slash[2], true);
    return buildDate(slash[3], slash[1], slash[2], true);
  }

  const yearless = parseYearlessDate(text, referenceDate);
  if (yearless) return yearless;

  const parsed = new Date(`${text} UTC`);
  if (!Number.isNaN(parsed.valueOf())) {
    return { date: parsed.toISOString().slice(0, 10), present: true };
  }
  return { date: "", present: true };
}

function parseYearlessDate(text, referenceDate) {
  const monthNames = {
    jan: 1,
    january: 1,
    feb: 2,
    february: 2,
    mar: 3,
    march: 3,
    apr: 4,
    april: 4,
    may: 5,
    jun: 6,
    june: 6,
    jul: 7,
    july: 7,
    aug: 8,
    august: 8,
    sep: 9,
    sept: 9,
    september: 9,
    oct: 10,
    october: 10,
    nov: 11,
    november: 11,
    dec: 12,
    december: 12
  };
  const named = text.match(/^([A-Za-z]+)\s+(\d{1,2})(?:st|nd|rd|th)?$/);
  const numeric = text.match(/^(\d{1,2})[/-](\d{1,2})$/);
  let month = 0;
  let day = 0;
  if (named) {
    month = monthNames[named[1].toLowerCase()] || 0;
    day = Number(named[2]);
  } else if (numeric) {
    month = Number(numeric[1]);
    day = Number(numeric[2]);
  } else {
    return null;
  }
  if (!month || month > 12 || !day || day > 31) return null;

  const reference = new Date(
    referenceDate
      ? `${String(referenceDate).slice(0, 10)}T00:00:00.000Z`
      : new Date().toISOString()
  );
  if (Number.isNaN(reference.valueOf())) return null;
  let year = reference.getUTCFullYear();
  let candidate = new Date(Date.UTC(year, month - 1, day));
  if (
    candidate.getUTCMonth() !== month - 1 ||
    candidate.getUTCDate() !== day
  ) {
    return null;
  }
  if (candidate.getTime() - reference.getTime() > 31 * 86400000) {
    year -= 1;
    candidate = new Date(Date.UTC(year, month - 1, day));
  }
  return { date: candidate.toISOString().slice(0, 10), present: true };
}

function buildDate(year, month, day, present) {
  const candidate = new Date(Date.UTC(Number(year), Number(month) - 1, Number(day)));
  if (
    candidate.getUTCFullYear() === Number(year) &&
    candidate.getUTCMonth() === Number(month) - 1 &&
    candidate.getUTCDate() === Number(day)
  ) {
    return { date: candidate.toISOString().slice(0, 10), present };
  }
  return { date: "", present };
}

export function addDays(dateText, days) {
  if (!dateText) return "";
  const date = new Date(`${dateText}T00:00:00.000Z`);
  if (Number.isNaN(date.valueOf())) return "";
  date.setUTCDate(date.getUTCDate() + days);
  return date.toISOString().slice(0, 10);
}

export function daysBetween(a, b) {
  if (!a || !b) return 0;
  const aDate = new Date(`${a}T00:00:00.000Z`);
  const bDate = new Date(`${b}T00:00:00.000Z`);
  if (Number.isNaN(aDate.valueOf()) || Number.isNaN(bDate.valueOf())) return 0;
  return Math.floor((bDate - aDate) / 86400000);
}

export function parseShare(value) {
  const text = String(value || "").trim();
  if (!text) return null;
  if (NO_CLEAR_RESULT_RE.test(text)) return "no_clear_winner";
  const candidate = text.replace(/%/g, "").replace(",", ".").trim();
  const number = Number(candidate);
  if (!Number.isFinite(number)) return text;
  if (text.includes("%") || number > 1) return number / 100;
  return number;
}

export function inferWinner(shares, options = {}) {
  const shareAnalysis = analyzeShares({ shares, options });
  const sheetResult = classifySheetResult({ shares });
  const hasNumericShares = Object.values(shares || {}).some(Number.isFinite);
  if (sheetResult.result !== "unknown") {
    const explicitWinner = sheetResult.explicitWinnerVariant || "";
    return {
      suggestedWinner: explicitWinner,
      detectedOutcome:
        sheetResult.result === "winner" && explicitWinner
          ? `winner_${explicitWinner.toLowerCase()}`
          : sheetResult.result === "inconclusive"
            ? "no_clear"
            : sheetResult.result,
      winnerReason:
        sheetResult.result === "winner"
          ? `Sheet explicitly reports option ${explicitWinner} as the winner.`
          : sheetResult.inconclusiveReason === "insufficient_views"
            ? "Sheet explicitly reports insufficient views."
            : "Sheet explicitly reports no winner.",
      resultEntered: true,
      ...sheetResult,
      ...shareAnalysis
    };
  }
  if (hasNumericShares) {
    const highestValue = shareAnalysis.highestShareVariant
      ? shares[shareAnalysis.highestShareVariant]
      : null;
    return {
      suggestedWinner: "",
      detectedOutcome: "finished_unknown",
      winnerReason: shareAnalysis.highestShareVariant
        ? `Highest watch-time share: ${(highestValue * 100).toFixed(1)}% (descriptive, not a YouTube result).`
        : "Watch-time shares are tied or incomplete; no YouTube result was inferred.",
      resultEntered: true,
      result: "unknown",
      resultEvidence: "unknown",
      resultSemanticsVersion: RESULT_SEMANTICS_VERSION,
      explicitWinnerVariant: "",
      inconclusiveReason: "",
      inconclusiveReasonEvidence: "",
      ...shareAnalysis
    };
  }
  return {
    suggestedWinner: "",
    detectedOutcome: "result_missing",
    winnerReason: "",
    resultEntered: false,
    result: "unknown",
    resultEvidence: "unknown",
    resultSemanticsVersion: RESULT_SEMANTICS_VERSION,
    explicitWinnerVariant: "",
    inconclusiveReason: "",
    inconclusiveReasonEvidence: "",
    ...shareAnalysis
  };
}

export function detectTestType(headers, sourceKind = "") {
  const hasThumbnailPair = hasHeader(headers, thumbnailOptionHeaderNames("A")) && hasHeader(headers, thumbnailOptionHeaderNames("B"));
  const hasTitlePair = hasHeader(headers, titleOptionHeaderNames("A")) && hasHeader(headers, titleOptionHeaderNames("B"));
  if (sourceKind === "title" && hasTitlePair) {
    return "title";
  }
  if (sourceKind === "thumbnail" && hasThumbnailPair) {
    return "thumbnail";
  }
  if (hasHeader(headers, ["thumbnail a", "a thumbnail"]) && hasHeader(headers, ["thumbnail b", "b thumbnail"])) {
    return "thumbnail";
  }
  if (hasHeader(headers, ["title a", "a title"]) && hasHeader(headers, ["title b", "b title"])) {
    return "title";
  }
  if (
    sourceKind === "thumbnail" &&
    hasHeader(headers, ["Video URL", "Video Link", "YouTube URL", "Studio URL"]) &&
    hasHeader(headers, ["Thumbnail", "Thumb", "Image", "Image A", "Option A"])
  ) {
    return "thumbnail";
  }
  return null;
}

export function parseWorkbookRecords({ spreadsheetId, sourceKind, sheets, today }) {
  return sheets.flatMap((sheet) =>
    parseSheetRecords({
      spreadsheetId: sheet.spreadsheetId || spreadsheetId,
      externalTabId: sheet.externalTabId || "",
      sourceKind,
      sheetName: sheet.title || sheet.name || "",
      values: sheet.values || [],
      today
    })
  );
}

export function inspectWorkbookSheets({ sourceKind, sheets }) {
  return (sheets || []).map((sheet) => {
    const values = sheet.values || [];
    const headerRow = values.length ? findHeaderRow(values, sourceKind) : null;
    const hasContent = values.some((row) =>
      (row || []).some((cell) => String(cell || "").trim() && !String(cell || "").trim().startsWith("="))
    );
    return {
      title: sheet.title || sheet.name || "",
      spreadsheetId: sheet.spreadsheetId || "",
      externalTabId: sheet.externalTabId || "",
      linkedFrom: sheet.linkedFrom || "",
      readStatus: sheet.readStatus || (sheet.readError ? "failed" : sheet.skippedByPolicy ? "skipped" : "fresh"),
      readError: sheet.readError || "",
      skippedByPolicy: Boolean(sheet.skippedByPolicy),
      rowCount: values.length,
      hasContent,
      recognized: Boolean(headerRow),
      likelyTestData: hasContent && !headerRow ? looksLikeTestData(values, sourceKind) : Boolean(headerRow),
      testType: headerRow?.testType || "",
      headerRow: headerRow ? headerRow.rowIndex + 1 : 0
    };
  });
}

function looksLikeTestData(values, sourceKind) {
  const rows = firstNonEmptyRows(values, 200);
  const cells = rows.flatMap((row) => (row || []).slice(0, 30).map((cell) => String(cell || "").trim()));
  const text = cells.filter(Boolean).join(" ").toLowerCase();
  if (!text) return false;
  const hasYoutubeVideo = /(?:youtube\.com\/(?:watch|shorts)|youtu\.be\/|studio\.youtube\.com\/video\/)/i.test(text);
  const hasLifecycle = /\b(?:start date|finish date|end date|published date|watch time|winner|done|completed)\b/i.test(text);
  const hasTitleOptions = /\btitle\s*a\b/i.test(text) && /\btitle\s*b\b/i.test(text);
  const hasThumbnailOptions = /\bthumbnail\s*a\b/i.test(text) && /\bthumbnail\s*b\b/i.test(text);
  const hasGenericOptions = /\boption\s*a\b/i.test(text) && /\boption\s*b\b/i.test(text);
  if (sourceKind === "title" && hasTitleOptions) return true;
  if (sourceKind === "thumbnail" && hasThumbnailOptions) return true;
  return hasYoutubeVideo && (hasLifecycle || hasGenericOptions || hasTitleOptions || hasThumbnailOptions);
}

export function parseSheetRecords({ spreadsheetId, externalTabId = "", sourceKind, sheetName, values, today }) {
  if (!values?.length) return [];
  const headerRow = findHeaderRow(values, sourceKind);
  if (!headerRow) return [];
  const { headers, rowIndex: headerRowIndex, testType } = headerRow;

  const startIdx = headerIndex(headers, [
    "Published Date/ Test Start Date",
    "Test Start / Published Date",
    "Test Start Date",
    "Start Date",
    "Published Date",
    "Publish Date"
  ]);
  const finishIdx = headerIndex(headers, ["Test Finish Date", "Test End Date", "End Date", "Finish Date"]);
  const doneIdx = headerIndex(headers, ["Done", "Status", "Completed"]);
  const urlIdx = headerIndex(headers, ["Video URL", "Video Link", "YouTube URL", "Youtube URL", "URL", "Studio URL"]);
  const titleIdx = headerIndex(headers, [
    "Video Title",
    "Title",
    "Current Title",
    "Current Title (Title A)",
    "Current YouTube Title"
  ]);
  const optionIndices = optionColumns(headers, testType);
  const shareIndices = shareColumns(headers);

  return values.slice(headerRowIndex + 1).flatMap((row, offset) => {
    if (!rowRepresentsTest(row, { startIdx, finishIdx, doneIdx, optionIndices, today })) {
      return [];
    }
    return [
      buildTestRun({
        spreadsheetId,
        externalTabId,
        sourceKind,
        sheetName,
        rowNumber: headerRowIndex + offset + 2,
        row,
        headers,
        testType,
        today,
        startIdx,
        finishIdx,
        doneIdx,
        urlIdx,
        titleIdx,
        optionIndices,
        shareIndices
      })
    ];
  });
}

function optionColumns(headers, testType) {
  return Object.fromEntries(
    ["A", "B", "C"]
      .map((option) => [
        option,
        headerIndex(
          headers,
          testType === "thumbnail" ? thumbnailOptionHeaderNames(option) : titleOptionHeaderNames(option)
        )
      ])
      .filter(([, idx]) => idx !== null)
  );
}

function shareColumns(headers) {
  return Object.fromEntries(
    ["A", "B", "C"]
      .map((option) => [
        option,
        headerIndex(headers, [
          `${option} - Watch-Time Share`,
          `${option} Watch-Time Share`,
          `${option} Watch Time Share`,
          `Watch-Time Share ${option}`,
          `Watch Time Share ${option}`,
          `${option} Share`,
          `${option} Result`
        ])
      ])
      .filter(([, idx]) => idx !== null)
  );
}

function findHeaderRow(values, sourceKind) {
  let nonEmptyRows = 0;
  for (let rowIndex = 0; rowIndex < values.length && nonEmptyRows < 200; rowIndex += 1) {
    const headers = values[rowIndex] || [];
    if (!headers.some((cell) => String(cell ?? "").trim())) continue;
    nonEmptyRows += 1;
    const testType = detectTestType(headers, sourceKind);
    if (testType) return { rowIndex, headers, testType };
  }
  return null;
}

function hasHeader(headers, names) {
  return headerIndex(headers, names) !== null;
}

function thumbnailOptionHeaderNames(option) {
  return [
    `Thumbnail ${option}`,
    `${option} Thumbnail`,
    `${option} - Thumbnail`,
    `Thumbnail ${option} URL`,
    `${option} Thumbnail URL`,
    `Image ${option}`,
    `${option} Image`,
    `Option ${option}`,
    `Variant ${option}`,
    option
  ];
}

function titleOptionHeaderNames(option) {
  return [
    `Title ${option}`,
    `${option} Title`,
    `${option} - Title`,
    `Option ${option}`,
    `Variant ${option}`
  ];
}

function rowRepresentsTest(row, { startIdx, finishIdx, doneIdx, optionIndices, today }) {
  const videoId = extractVideoId(...row);
  if (videoId) return true;
  const optionCount = Object.values(optionIndices).filter((idx) => {
    const text = String(row[idx] || "").trim();
    return text && !text.startsWith("=");
  }).length;
  if (optionCount < 2) return false;
  const hasLifecycleSignal = Boolean(
    parseDate(valueAt(row, startIdx), today).date ||
    parseDate(valueAt(row, finishIdx), today).date ||
    parseBool(valueAt(row, doneIdx))
  );
  return hasLifecycleSignal;
}

function buildTestRun(args) {
  const {
    spreadsheetId,
    externalTabId,
    sourceKind,
    sheetName,
    rowNumber,
    row,
    headers,
    testType,
    today,
    startIdx,
    finishIdx,
    doneIdx,
    urlIdx,
    titleIdx,
    optionIndices,
    shareIndices
  } = args;
  const padded = Array.from({ length: Math.max(headers.length, row.length) }, (_, idx) => row[idx] ?? "");
  const videoId = extractVideoId(...padded);
  const start = parseDate(valueAt(padded, startIdx), today);
  const finish = parseDate(valueAt(padded, finishIdx), today);
  const startDate = start.date;
  const finishDate = finish.date;
  const effectiveFinishDate = finishDate;
  const done = parseBool(valueAt(padded, doneIdx));
  const options = Object.fromEntries(
    Object.entries(optionIndices)
      .map(([option, idx]) => [option, valueAt(padded, idx)])
      .filter(([, value]) => value)
  );
  const watchTimeShare = Object.fromEntries(
    Object.entries(shareIndices).map(([option, idx]) => [option, parseShare(valueAt(padded, idx))])
  );
  const winner = inferWinner(watchTimeShare, options);
  const troubles = detectTroubles({
    videoId,
    start,
    finish,
    testType,
    options,
    done
  });
  const status = classifyStatus({
    done,
    troubles,
    startDate,
    effectiveFinishDate,
    today,
    resultEntered: winner.resultEntered,
    detectedOutcome: winner.detectedOutcome
  });
  const optionFingerprint = hashStable(options);
  const sourcePayload = {
    spreadsheetId,
    externalTabId,
    sourceKind,
    sheetName,
    rowNumber,
    headers,
    row: padded
  };
  const relevantSourceState = {
    videoId,
    videoTitle: valueAt(padded, titleIdx),
    videoUrl: valueAt(padded, urlIdx),
    startDate,
    finishDate,
    done,
    options,
    watchTimeShare
  };
  const rowFingerprint = hashStable(relevantSourceState);
  const contentHash = testContentHash({
    videoId,
    testType,
    startDate,
    options
  });
  const testRunId = makeTestRunId({
    spreadsheetId,
    sheetName,
    rowNumber,
    testType,
    videoId,
    startDate,
    finishDate,
    optionFingerprint
  });
  return {
    testRunId,
    videoId,
    spreadsheetId,
    externalTabId,
    sourceKind,
    sheetName,
    rowNumber,
    testType,
    channel: canonicalChannelName(sheetName) || sheetName,
    videoTitle: valueAt(padded, titleIdx),
    videoUrl: cleanVideoUrl(valueAt(padded, urlIdx), videoId),
    studioUrl: extractStudioUrl(...padded) || (videoId ? `https://studio.youtube.com/video/${videoId}/edit` : ""),
    startDate,
    finishDate,
    effectiveFinishDate,
    overdueDays:
      effectiveFinishDate && effectiveFinishDate < today && ["needs_review"].includes(status)
        ? daysBetween(effectiveFinishDate, today)
        : 0,
    options,
    watchTimeShare,
    suggestedWinner:
      winner.suggestedWinner || (winner.detectedOutcome === "no_clear" ? "No clear winner" : ""),
    detectedOutcome: winner.detectedOutcome,
    winnerReason: winner.winnerReason,
    result: winner.result,
    resultEvidence: winner.resultEvidence,
    resultSemanticsVersion: winner.resultSemanticsVersion,
    explicitWinnerVariant: winner.explicitWinnerVariant,
    highestShareVariant: winner.highestShareVariant,
    configuredVariantCount: winner.configuredVariantCount,
    populatedShareCount: winner.populatedShareCount,
    shareSum: winner.shareSum,
    shareSumValid: winner.shareSumValid,
    shareQuality: winner.quality,
    operationalDecision: "",
    youtubeAppliedVariant: "",
    inconclusiveReason: winner.inconclusiveReason,
    inconclusiveReasonEvidence: winner.inconclusiveReasonEvidence,
    contentHash,
    status,
    troubles,
    optionFingerprint,
    rowFingerprint,
    sourcePayloadHash: hashStable(relevantSourceState),
    sourcePayload,
    currentYoutubeTitle: "",
    currentYoutubeThumbnailUrl: "",
    youtubeChannelId: "",
    youtubeChannelTitle: "",
    youtubeChannelThumbnailUrl: "",
    thumbnailPreviews: {}
  };
}

function detectTroubles({ videoId, start, finish, testType, options, done }) {
  const troubles = [];
  if (!videoId && !done) {
    troubles.push({
      severity: "error",
      code: "missing_video_id",
      message: "Could not find a YouTube video ID in the row.",
      suggestedFix: "Paste a youtu.be, youtube.com/watch, or Studio edit URL into the row."
    });
  }
  if (!start.date && !done) {
    troubles.push({
      severity: "error",
      code: start.present ? "unparseable_start_date" : "missing_start_date",
      message: "The row has no usable test start date.",
      suggestedFix: "Fill the test start date in the first date column."
    });
  }
  if (finish.present && !finish.date && !done) {
    troubles.push({
      severity: "warning",
      code: "unparseable_finish_date",
      message: "The test finish date is present but could not be parsed.",
      suggestedFix: "Use a standard date such as 2026-06-22."
    });
  }
  if (testType === "title" && Object.keys(options).length < 2 && !done) {
    troubles.push({
      severity: "error",
      code: "missing_options",
      message: "The row needs at least two A/B options.",
      suggestedFix: "Fill at least option A and B."
    });
  }
  return troubles;
}

export function classifyStatus({ done, troubles, startDate, effectiveFinishDate, today, resultEntered, detectedOutcome }) {
  if (done) return "sheet_marked_done";
  if (resultEntered) return "result_logged";
  if (troubles.some((trouble) => trouble.severity === "error")) return "missing_data";
  if (effectiveFinishDate && effectiveFinishDate <= today) return "needs_review";
  if (startDate && today && startDate > today) return "scheduled";
  return "running";
}

function valueAt(row, idx) {
  if (idx === null || idx === undefined || idx >= row.length) return "";
  return String(row[idx] ?? "").trim();
}

function firstNonEmptyRows(values, limit) {
  const rows = [];
  for (const row of values || []) {
    if (!(row || []).some((cell) => String(cell ?? "").trim())) continue;
    rows.push(row);
    if (rows.length >= limit) break;
  }
  return rows;
}

export function extractStudioUrl(...values) {
  const joined = values.map((value) => String(value || "")).join(" ");
  const match = joined.match(/https:\/\/studio\.youtube\.com\/video\/[A-Za-z0-9_-]{6,}\/edit[^\s"]*/);
  return match ? match[0] : "";
}

export function cleanVideoUrl(rawValue, videoId) {
  const text = String(rawValue || "");
  const match = text.match(
    /https?:\/\/(?:www\.)?(?:youtube\.com\/(?:watch\?[^\s"']*v=|shorts\/|live\/|embed\/)|youtu\.be\/)[A-Za-z0-9_-]{6,}[^\s"']*/i
  );
  if (match) return match[0];
  if (videoId) return `https://www.youtube.com/watch?v=${videoId}`;
  return text.trim();
}

export function makeTestRunId({ spreadsheetId, sheetName, rowNumber, testType, videoId, startDate, optionFingerprint }) {
  return sha1([
    "v2",
    spreadsheetId,
    sheetName,
    rowNumber,
    testType,
    videoId,
    startDate,
    optionFingerprint
  ].join("|")).slice(0, 20);
}

export function hashStable(value) {
  return sha1(JSON.stringify(sortDeep(value)));
}

function sha1(value) {
  return crypto.createHash("sha1").update(String(value), "utf8").digest("hex");
}

function sortDeep(value) {
  if (Array.isArray(value)) return value.map(sortDeep);
  if (value && typeof value === "object") {
    return Object.fromEntries(
      Object.entries(value)
        .sort(([a], [b]) => a.localeCompare(b))
        .map(([key, item]) => [key, sortDeep(item)])
    );
  }
  return value;
}
