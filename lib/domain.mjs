...
export const FINISH_FALLBACK_DAYS = 14;
...
export function detectTestType(headers) {
  const normalized = headers.map(normalizeHeader);
  if (normalized.includes("thumbnail a") && normalized.includes("thumbnail b")) {
    return "thumbnail";
  }
  if (normalized.includes("title a") && normalized.includes("title b")) {
    return "title";
  }
  return null;
}
...
export function parseSheetRecords({ spreadsheetId, sourceKind, sheetName, values, today }) {
  ...
  const testType = detectTestType(headers);
  if (!testType) return [];
  ...
  const finishIdx = headerIndex(headers, ["Test Finish Date"]);
  const doneIdx = headerIndex(headers, ["Done"]);
  const urlIdx = headerIndex(headers, ["Video URL"]);
  ...
  return values.slice(1).flatMap((row, offset) => {
...
function optionColumns(headers, testType) {
  const prefix = testType === "thumbnail" ? "Thumbnail" : "Title";
  return Object.fromEntries(
    ["A", "B", "C"]
      .map((option) => [option, headerIndex(headers, [`${prefix} ${option}`])])
      .filter(([, idx]) => idx !== null)
  );
}
...
function shareColumns(headers) {
  return Object.fromEntries(
    ["A", "B", "C"]
      .map((option) => [
        option,
        headerIndex(headers, [`${option} - Watch-Time Share`, `${option} Watch-Time Share`])
      ])
      .filter(([, idx]) => idx !== null)
  );
}
...
function buildTestRun(args) {
  ...
  const effectiveFinishDate = finishDate || addDays(startDate, FINISH_FALLBACK_DAYS);
