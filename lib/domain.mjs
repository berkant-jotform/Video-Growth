...
export function inferWinner(shares) {
  ...
  if (numeric.length) {
    const sorted = [...numeric].sort((a, b) => b[1] - a[1]);
    if (sorted.length > 1 && Math.abs(sorted[0][1] - sorted[1][1]) < 0.000001) {
      return {
        suggestedWinner: "",
        detectedOutcome: "no_clear",
        winnerReason: `Top watch-time shares are tied at ${(sorted[0][1] * 100).toFixed(1)}%.`,
        resultEntered: true
      };
    }
    const [winner, value] = sorted[0];
    return {
