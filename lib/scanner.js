...
export async function runScan({ actorName = "system", channel = "all", channels = [], testType = "all", refreshThumbnails = false } = {}) {
  ...
  try {
    ...
    for (const source of [
      ...
      const sheetInspection = inspectWorkbookSheets({ sourceKind: source.sourceKind, sheets });
      const skippedTabs = sheetInspection.filter((sheet) => sheet.hasContent && !sheet.recognized);
      if (skippedTabs.length) {
        warnings.push(
          `${source.sourceKind} sheet has ${skippedTabs.length} non-empty tab${skippedTabs.length === 1 ? "" : "s"} without recognizable A/B headers.`
        );
        await recordDiagnosticLog({
    ...
    if (revalidatedEvents.length) {
      warnings.push(`Revalidated ${revalidatedEvents.length} uncertain or noisy Studio signals.`);
    }
    ...
    if (rematchedEvents.length) {
      warnings.push(`Auto-matched ${rematchedEvents.length} previously unmatched Studio finish signals.`);
    }
    await progress(
      "finish_signals",
      "Finalizing queue",
