...
export async function runScan({ actorName = "system", channel = "all", testType = "all" } = {}) {
  ...
  try {
    await progress(
      ...
      "Preparing scan",
      4,
      partialScan ? "Preparing filtered scan. Other channels will not be marked missing." : "Cleaning old inline thumbnail data."
    );
    ...
    if (config.thumbnailSpreadsheetId && config.blobReadWriteToken && (!filters.testType || filters.testType === "thumbnail")) {
      try {
        await progress(
          "thumbnail_previews",
