...
import { inactiveSourceTabs, sourceTabKey, sourceTabLookupKeys, sourceTabPolicy } from "@/lib/source-tabs.mjs";
import {
  completeScanRun,
...
export async function runScan({ actorName = "system", scanId: requestedScanId = "", channel = "all", channels = [], testType = "all", refreshThumbnails = false } = {}) {
  ...
  try {
    ...
    for (const source of [
      ...
      const inactiveSheetKeys = new Set();
      for (const sheet of allSheetInspection) {
        ...
        if (policy.mode !== "active") {
          inactiveSheetKeys.add(sourceTabKey(source.sourceKind, sheet.title, sheet.spreadsheetId || source.spreadsheetId, sheet.externalTabId));
        }
      }
      const includedSheets = inactiveSheetKeys.size
        ? sheets.filter((sheet) => !inactiveSheetKeys.has(sourceTabKey(source.sourceKind, sheet.title, sheet.spreadsheetId || source.spreadsheetId, sheet.externalTabId)))
        : sheets;
      ...
      const parsedRowsByTab = parsed.reduce((map, record) => {
        const key = sourceTabKey(source.sourceKind, record.sheetName, record.spreadsheetId, record.externalTabId);
      ...
      for (const sheet of allSheetInspection) {
        const sheetKey = sourceTabKey(source.sourceKind, sheet.title, sheet.spreadsheetId || source.spreadsheetId, sheet.externalTabId);
        for (const candidateKey of sourceTabLookupKeys(
          source.sourceKind,
          sheet.title,
          sheet.spreadsheetId || source.spreadsheetId,
          sheet.externalTabId
        )) discoveredSourceTabKeys.add(candidateKey);
        const policy = sourceTabPolicy(
          {
            sourceKind: source.sourceKind,
            spreadsheetId: sheet.spreadsheetId || source.spreadsheetId,
        ...
        let testRows = parsedRowsByTab.get(sheetKey) || 0;
        if (policy.mode !== "active" && sheet.recognized) {
          const sourceSheet = sheets.find((item) => sourceTabKey(source.sourceKind, item.title, item.spreadsheetId || source.spreadsheetId, item.externalTabId) === sheetKey);
          if (sourceSheet) {
        ...
        counts.sheetTabs.push({
          ...
          testRows,
          monitoringRows: monitoringRowsByTab.get(sheetKey) || 0,
          readyToArchive: policy.mode === "active" && testRows > 0 && (monitoringRowsByTab.get(sheetKey) || 0) === 0
        });
      }
      reconciliationScopes.push(...await timed(`inventory_${source.sourceKind}`, () => syncSourceInventory({
        sourceKind: source.sourceKind,
        scanId,
        workbookReports: sheets.workbookReports || [{ spreadsheetId: source.spreadsheetId, status: sheets.readIncomplete ? "partial" : "fresh", tabs: [] }],
        tabs: counts.sheetTabs.filter((tab) => tab.sourceKind === source.sourceKind)
      })));
    ...
    const stalePolicies = config.sourceTabPolicies.filter(
      (item) => completelyReadKinds.has(item.sourceKind) && !discoveredSourceTabKeys.has(sourceTabKey(item.sourceKind, item.sheetName, item.spreadsheetId, item.externalTabId))
    );
    ...
    for (const source of [
      ...
      for (const item of inactiveSourceTabs(config.sourceTabPolicies, config.excludedSheetTabs, source.sourceKind)) {
        sourceTabModes.push({ ...item, spreadsheetId: item.spreadsheetId || source.spreadsheetId });
      }
    }
    ...
    counts.channelRegistry = await timed("channel_registry", () => syncChannelRegistry(enriched.records));
    ...
    const repairedLinkedIdentities = await timed("repair_linked_identities", () =>
      repairLinkedRecordIdentities()
    );
    counts.repairedEventIdentities = repairedLinkedIdentities.events;
    counts.repairedActionIdentities = repairedLinkedIdentities.actions;
    if (repairedLinkedIdentities.events || repairedLinkedIdentities.actions) {
      notices.push(
        `Repaired ${repairedLinkedIdentities.events} finish-event and ${repairedLinkedIdentities.actions} reviewer-action identity link${repairedLinkedIdentities.events + repairedLinkedIdentities.actions === 1 ? "" : "s"}.`
      );
    }
    ...
    if (!partialScan) {
      await timed("reconcile_missing", () => reconcileMissingRuns({ scanId, scopes: reconciliationScopes }));
    }
