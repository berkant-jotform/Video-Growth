      ...
      `
        ...
        SET possible_retest = EXISTS (
          ...
          WHERE other.video_id = current.video_id
            ...
            AND (
              other.start_date IS DISTINCT FROM current.start_date
              OR other.option_fingerprint IS DISTINCT FROM current.option_fingerprint
            )
        )
  ...
  const rows = await sql.query(
    `
      SELECT
        ...
        tr.updated_at,
        EXISTS (SELECT 1 FROM test_actions ta WHERE ta.test_run_id = tr.test_run_id) AS has_action
      FROM test_runs tr
  ...
  for (const sourceKind of sourceKinds) {
    await sql`
      ...
      WHERE tr.source_kind = ${sourceKind}
        ...
        AND NOT EXISTS (SELECT 1 FROM test_actions ta WHERE ta.test_run_id = tr.test_run_id)
    `;
    await sql`
      UPDATE test_runs tr
...
export async function completeTestRun({ testRunId, action, actorName, note, retestConfirmed }) {
  ...
  if (existing[0].recent_action === action) return getTestRun(testRunId);
...
export async function resolveReviewItem({ targetType, targetId, action = "ignore", actorName = "", note = "", metadata = {} }) {
  ...
  await sql`
    ...
    DO UPDATE SET
      ...
      created_at = NOW()
  `;
  return { resolutionId, targetType, targetId, action };
}
    ...
    ORDER BY
      CASE
        WHEN tr.status IN ('sheet_marked_done', 'result_logged', 'winner_found', 'no_clear')
          OR EXISTS (SELECT 1 FROM test_actions ta WHERE ta.test_run_id = tr.test_run_id)
        THEN 1
        ELSE 0
      END,
      LOWER(tr.channel),
...
async function ensureAppManagedRun({ sql, event, youtubeApiKey = "" }) {
  ...
  const identity = appManagedRunIdentity(event);
  ...
  const testRunId = identity.testRunId;
