...
export async function undoTestAction({ actionId, actorName = "Reviewer" }) {
  ...
  const sql = getSql();
  const rows = await sql`
    SELECT ta.action_id, ta.test_run_id, tr.test_id
    FROM test_actions ta
    JOIN test_runs tr ON tr.test_run_id = ta.test_run_id
...
export async function reconcileLegacyYearlessDateRuns({ apply = true, rehearse = false } = {}) {
  ...
  const candidateRows = await sql`
    ...
    WHERE legacy.start_date < DATE '2005-01-01'
      ...
      AND target.test_id IS NOT NULL
      AND legacy.test_id <> target.test_id
  `;
  ...
  const planned = planLegacyYearlessDateReconciliation(candidateRows.map((row) => ({
    ...
    testType: row.test_type,
    optionFingerprint: row.option_fingerprint,
    legacyStartDate: dateOnly(row.legacy_start_date),
    targetStartDate: dateOnly(row.target_start_date),
  ...
  try {
    ...
    txn`
      ...
      SET test_id = mapping.target_test_id,
          status = CASE WHEN run.test_run_id = mapping.legacy_run_id THEN 'source_removed' ELSE run.status END,
          drift_reason = CASE
            WHEN run.test_run_id = mapping.legacy_run_id
              THEN 'legacy_yearless_date_reconciled:' || mapping.target_run_id
            ELSE run.drift_reason
          END,
          updated_at = NOW()
      FROM mapping
