...
async function assignPersistentTestIds(sql, records) {
  ...
  for (let index = 0; index < allAliases.length; index += aliasBatchSize) {
    ...
    for (const row of rows) aliasToTestId.set(row.alias_value, row.test_id);
  }
  ...
  for (const record of records) {
    ...
    for (const alias of identityAliases(record)) aliasToTestId.set(alias, record.testId);
