  ...
  await sql.query(
    `
      ...
      INSERT INTO test_runs (
        test_run_id, video_id, source_kind, spreadsheet_id, sheet_name, row_number,
        ...
        winner_reason, options, watch_time_share, troubles, thumbnail_previews,
...
async function reuseExistingRunIds(sql, records) {
  ...
  for (const row of rows) {
    ...
    if (!byNaturalKey.has(key)) byNaturalKey.set(key, row.test_run_id);
  }
  for (const record of records) {
    const existingId = byNaturalKey.get(naturalRunKey(record));
    if (existingId) record.testRunId = existingId;
  }
}
    ...
    latest_action AS (
      ...
      JOIN test_runs source
        ...
        OR (
          ...
          AND (
            source.spreadsheet_id IS DISTINCT FROM target.spreadsheet_id
            OR source.sheet_name IS DISTINCT FROM target.sheet_name
            OR source.row_number = target.row_number
          )
        )
    ...
    WHERE tr.status <> 'source_removed'
      AND NOT EXISTS (
        ...
        WHERE closed_copy.test_run_id <> tr.test_run_id
          ...
          AND (
            closed_copy.spreadsheet_id IS DISTINCT FROM tr.spreadsheet_id
            OR closed_copy.sheet_name IS DISTINCT FROM tr.sheet_name
            OR closed_copy.row_number = tr.row_number
          )
...
export async function completeTestRun({ testRunId, action, actorName, note, retestConfirmed }) {
  ...
  const existing = await sql`
    SELECT tr.test_run_id,
      (
        ...
        FROM test_actions ta
        WHERE ta.test_run_id = tr.test_run_id
          AND ta.actor_name = ${actorName || "Reviewer"}
          AND ta.undone_at IS NULL
          AND ta.created_at > NOW() - INTERVAL '10 seconds'
        ORDER BY ta.created_at DESC
        LIMIT 1
      ) AS recent_action
    FROM test_runs tr
  ...
  if (existing[0].recent_action === action) {
    const test = await getTestRun(testRunId);
    const latest = test?.actions?.find((item) => !item.undone_at);
    return { test, actionId: latest?.action_id || "", duplicate: true };
  }
  ...
  `;
  return { test: await getTestRun(testRunId), actionId, duplicate: false };
}
...
export async function undoTestAction({ actionId, actorName = "Reviewer" }) {
  ...
  const newer = await sql`
    SELECT action_id
    FROM test_actions
    WHERE test_run_id = ${rows[0].test_run_id}
      AND undone_at IS NULL
...
export async function listConnectorActiveRuns() {
  ...
  const rows = await sql`
    SELECT
      ...
      tr.updated_at
    FROM test_runs tr
    WHERE tr.status NOT IN ('sheet_marked_done', 'result_logged', 'winner_found', 'no_clear', 'source_removed')
      AND NOT EXISTS (
        SELECT 1 FROM test_actions ta
        WHERE ta.test_run_id = tr.test_run_id AND ta.undone_at IS NULL
      )
...
async function listConnectorMatchRuns() {
  ...
  const rows = await sql`
    SELECT
      ...
      tr.updated_at
    FROM test_runs tr
    WHERE tr.status NOT IN ('missing_data', 'source_removed')
...
export async function revalidateMatchedFinishEvents({ limit = 500, youtubeApiKey = "" } = {}) {
  ...
  const rows = await sql`
    SELECT event_id, video_id, channel_id, channel, source, raw_text, notification_url,
      notification_age, detected_outcome, matched_confidence, payload, occurred_at, observed_at, test_run_id
    FROM finish_events
    WHERE processing_status = 'matched'
      AND source <> 'metadata'
      AND NOT EXISTS (
        SELECT 1 FROM test_runs managed
        WHERE managed.test_run_id = finish_events.test_run_id
          AND managed.source_kind = 'app_registry'
      )
      AND (
        matched_confidence ILIKE '%fuzzy%'
        OR COALESCE(payload->>'videoTitle', '') = ''
        OR raw_text ILIKE '%A/B Test completed%'
      )
    ORDER BY observed_at DESC
    LIMIT ${limit}
  `;
...
export async function reconcileAppManagedRunsWithSheets() {
  ...
  for (const link of links) {
    await sql`
      UPDATE finish_events
      SET test_run_id = ${link.sheet_run_id},
          matched_confidence = 'app_registry_to_sheet',
          processing_status = 'matched',
          updated_at = NOW()
      WHERE test_run_id = ${link.managed_run_id}
    `;
    await sql`
      UPDATE test_actions
      SET test_run_id = ${link.sheet_run_id},
          metadata = COALESCE(metadata, '{}'::jsonb) || jsonb_build_object('reconciledFrom', ${link.managed_run_id})
      WHERE test_run_id = ${link.managed_run_id}
    `;
    await sql`
      UPDATE test_runs
...
async function supersedeDuplicateFinishEvents() {
  ...
  await sql`
    WITH ranked AS (
      SELECT event_id,
        ROW_NUMBER() OVER (
          PARTITION BY test_run_id, detected_outcome
          ORDER BY
...
async function resolveFinishEventVideo(event, { youtubeApiKey = "" } = {}) {
  if (event.videoId || !event.videoTitle || !youtubeApiKey) {
  ...
  try {
    ...
    const candidate = candidates.find((item) => isTrustedYouTubeCandidate(event, item)) || null;
    if (!candidate) {
      return {
        event: {
          ...event,
    ...
    return {
      event: {
        ...
        channelId: event.channelId || candidate.channelId || "",
        channel: event.channel || candidate.channel || "",
        youtubeCandidates: candidates
      },
      summary: {
        source: "youtube_search",
...
async function ensureAppManagedRun({ sql, event, youtubeApiKey = "" }) {
  ...
  let metadata = {};
  if (event.videoId && youtubeApiKey && (!event.videoTitle || !event.channelId || !event.channel)) {
  ...
  const identity = appManagedRunIdentity({
    ...event,
    videoId: event.videoId || metadata.videoId || "",
    videoTitle: event.videoTitle || metadata.title || "",
    channelId: event.channelId || metadata.channelId || "",
    channel: metadata.channelTitle || event.channel || ""
  });
  const testType = identity.testType;
  const videoId = event.videoId || metadata.videoId || "";
  const videoTitle = event.videoTitle || metadata.title || videoId || "Finished A/B test";
  const channelId = event.channelId || metadata.channelId || "";
  const channel = canonicalChannelName(metadata.channelTitle || event.channel) || metadata.channelTitle || event.channel || "Unknown source";
  const existingRuns = await sql`
    SELECT test_run_id
    FROM test_runs
    WHERE source_kind = 'app_registry'
      AND status <> 'source_removed'
      AND test_type = ${testType}
      AND (
        (${videoId} <> '' AND video_id = ${videoId})
        OR (
          ${videoTitle} <> ''
          AND LOWER(TRIM(COALESCE(NULLIF(current_youtube_title, ''), video_title))) = LOWER(TRIM(${videoTitle}))
          AND (
            LOWER(TRIM(channel)) = LOWER(TRIM(${channel}))
            OR LOWER(TRIM(channel)) IN ('', 'unknown source')
            OR LOWER(TRIM(${channel})) IN ('', 'unknown source')
          )
        )
      )
  ...
  const studioUrl = videoId
    ...
    : event.url || event.notificationUrl || "";
  const detectedOutcome = event.detectedOutcome || "finished_unknown";
  const suggestedWinner = detectedOutcome === "no_clear" ? "No clear winner" : "";
  const sourcePayload = {
    registry: "app",
    event: {
      source: event.source || "studio_bell",
  ...
  await sql`
    INSERT INTO test_runs (
      ...
      suggested_winner, winner_reason, options, watch_time_share, troubles,
      ...
      ${testRunId}, ${videoId}, 'app_registry', '', 'App registry', 0,
...
function runRow(row) {
  ...
  return {
    ...
    finishEventOccurredAt: row.finish_event_occurred_at || "",
...
function activeQueueRun(run) {
  ...
  return !["done", "sheet_marked_done", "result_logged", "winner_found", "no_clear"].includes(run.queueStatus);
}
