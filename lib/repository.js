...
export async function rematchUnmatchedFinishEvents({ limit = 200 } = {}) {
  await ensureSchema();
  const sql = getSql();
  const matchRuns = await listConnectorMatchRuns();
  ...
  for (const row of rows) {
    ...
    await sql`
      ...
      SET test_run_id = ${match.run.testRunId},
          matched_confidence = ${`rematch_${match.matchedConfidence}`},
          processing_status = 'matched',
          updated_at = NOW()
    ...
    matched.push({
      eventId: row.event_id,
      testRunId: match.run.testRunId,
      matchedConfidence: `rematch_${match.matchedConfidence}`
    });
  }
  return matched;
}
...
function applyConnectorCoverage(run, coverage) {
  const covered = coverage.get(connectorCoverageKey(run.channel));
  const next = {
    ...run,
