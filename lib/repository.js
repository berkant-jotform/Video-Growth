...
import { deriveQueueStatus, resolveBaseQueueStatus } from "@/lib/queue-status.mjs";
...
import { projectCanonicalResult, RESULT_SEMANTICS_VERSION } from "@/lib/result-semantics.mjs";
...
function scannedRunDbRow({ record, scanId, driftedAt, driftReason, previousHash }) {
  return {
    ...
    source_tab_mode: "active",
    spreadsheet_id: record.spreadsheetId,
    external_tab_id: record.externalTabId || "",
    ...
    `
      WITH incoming AS (
        ...
        FROM jsonb_to_recordset($1::jsonb) AS row_data(
          ...
          video_id text,
          source_kind text,
          source_tab_mode text,
          spreadsheet_id text,
          external_tab_id text,
          sheet_name text,
          row_number integer,
          test_type text,
          channel text,
      ...
      INSERT INTO test_runs (
        test_run_id, test_id, content_hash, video_id, source_kind, source_tab_mode, spreadsheet_id, external_tab_id, sheet_name, row_number,
      ...
      SELECT
        test_run_id, NULLIF(test_id, ''), content_hash, video_id, source_kind, source_tab_mode, spreadsheet_id, external_tab_id, sheet_name, row_number,
      ...
      DO UPDATE SET
        ...
        external_tab_id = EXCLUDED.external_tab_id,
...
export async function reconcileMissingRuns({ scanId, sourceKinds = [], scopes } = {}) {
  await ensureSchema();
  const sql = getSql();
  if (Array.isArray(scopes)) {
    return reconcileMissingRunScopes(sql, scanId, scopes);
  }
...
async function reconcileMissingRunScopes(sql, scanId, scopes = []) {
  const normalized = scopes
    .map((scope) => ({
      source_kind: String(scope.sourceKind || "").trim().toLowerCase(),
      spreadsheet_id: String(scope.spreadsheetId || "").trim(),
      external_tab_id: String(scope.externalTabId || "").trim(),
      sheet_name: String(scope.sheetName || "").trim(),
      removed: Boolean(scope.removed)
    }))
    .filter((scope) => ["title", "thumbnail"].includes(scope.source_kind) && scope.spreadsheet_id && (scope.external_tab_id || scope.sheet_name));
  if (!normalized.length) return 0;
  const rows = await sql.query(
    `
      WITH scopes AS (
        SELECT *
        FROM jsonb_to_recordset($1::jsonb) AS scope(
          source_kind text,
          spreadsheet_id text,
          external_tab_id text,
          sheet_name text,
      ...
      SET status = 'source_removed',
          drifted_at = CASE
            WHEN EXISTS (SELECT 1 FROM test_actions ta WHERE ta.test_run_id = tr.test_run_id AND ta.undone_at IS NULL)
              THEN COALESCE(tr.drifted_at, NOW())
            ELSE tr.drifted_at
          END,
          drift_reason = CASE
            WHEN tr.drift_reason <> '' THEN tr.drift_reason
            WHEN EXISTS (SELECT 1 FROM test_actions ta WHERE ta.test_run_id = tr.test_run_id AND ta.undone_at IS NULL)
              THEN 'source_missing_after_done'
            ELSE 'source_removed'
          END,
          updated_at = NOW()
      FROM scopes
      WHERE tr.source_kind = scopes.source_kind
        AND tr.spreadsheet_id = scopes.spreadsheet_id
        AND (
          (scopes.external_tab_id <> '' AND (
            tr.external_tab_id = scopes.external_tab_id
            OR (tr.external_tab_id = '' AND LOWER(TRIM(tr.sheet_name)) = LOWER(TRIM(scopes.sheet_name)))
          ))
          OR (scopes.external_tab_id = '' AND LOWER(TRIM(tr.sheet_name)) = LOWER(TRIM(scopes.sheet_name)))
        )
        AND tr.source_tab_mode = 'active'
        AND (scopes.removed OR tr.last_seen_scan_id IS DISTINCT FROM $2)
      RETURNING tr.test_run_id
    `,
    [JSON.stringify(normalized), scanId]
  );
  return rows.length;
}

export async function syncSourceInventory({ sourceKind, scanId, workbookReports = [], tabs = [] }) {
  await ensureSchema();
  const sql = getSql();
  const normalizedKind = String(sourceKind || "").trim().toLowerCase();
  const reports = workbookReports
    .map((report) => ({
      source_kind: normalizedKind,
      spreadsheet_id: String(report.spreadsheetId || "").trim(),
      parent_spreadsheet_id: String(report.parentSpreadsheetId || "").trim(),
      linked_from: String(report.linkedFrom || "").trim(),
      read_status: String(report.status || "unknown").trim(),
      last_error: String(report.error || "").slice(0, 1000)
    }))
    .filter((report) => report.spreadsheet_id);
  const currentTabs = tabs
    .map((tab) => ({
      source_tab_key: sourceInventoryTabKey(normalizedKind, tab),
      source_kind: normalizedKind,
      spreadsheet_id: String(tab.spreadsheetId || "").trim(),
      external_tab_id: String(tab.externalTabId || "").trim(),
      current_title: String(tab.title || "").trim(),
      mode: String(tab.mode || "active").trim(),
      read_status: String(tab.readStatus || "unknown").trim(),
      recognized: Boolean(tab.recognized),
      likely_test_data: Boolean(tab.likelyTestData),
      last_error: String(tab.readError || "").slice(0, 1000)
    }))
    .filter((tab) => tab.spreadsheet_id && tab.current_title);

  const readableWorkbookIds = reports
    .filter((report) => ["fresh", "partial"].includes(report.read_status))
    .map((report) => report.spreadsheet_id);
  const previous = readableWorkbookIds.length
    ? await sql.query(
        `SELECT * FROM source_tabs WHERE source_kind = $1 AND spreadsheet_id = ANY($2::text[])`,
        [normalizedKind, readableWorkbookIds]
      )
    : [];
    ...
    await sql.query(
      `
        INSERT INTO source_workbooks (
          source_key, source_kind, spreadsheet_id, parent_spreadsheet_id, linked_from, read_status,
          last_seen_scan_id, last_read_at, last_error, updated_at
        )
        SELECT
          source_kind || '|' || spreadsheet_id, source_kind, spreadsheet_id, parent_spreadsheet_id, linked_from, read_status,
          $2, CASE WHEN read_status IN ('fresh', 'partial') THEN NOW() ELSE NULL END, last_error, NOW()
        FROM jsonb_to_recordset($1::jsonb) AS row_data(
          source_kind text, spreadsheet_id text, parent_spreadsheet_id text, linked_from text, read_status text, last_error text
        )
        ON CONFLICT (source_key) DO UPDATE SET
          parent_spreadsheet_id = EXCLUDED.parent_spreadsheet_id,
          linked_from = EXCLUDED.linked_from,
          read_status = EXCLUDED.read_status,
          last_seen_scan_id = EXCLUDED.last_seen_scan_id,
          last_read_at = COALESCE(EXCLUDED.last_read_at, source_workbooks.last_read_at),
          last_error = EXCLUDED.last_error,
          updated_at = NOW()
      `,
      [JSON.stringify(reports), scanId]
    );
  }
  if (currentTabs.length) {
    await sql.query(
      `
        INSERT INTO source_tabs (
          source_tab_key, source_kind, spreadsheet_id, external_tab_id, current_title,
          mode, read_status, recognized, likely_test_data, last_seen_scan_id,
          last_read_at, missing_since, last_error, updated_at
        )
        SELECT
          source_tab_key, source_kind, spreadsheet_id, external_tab_id, current_title,
          mode, read_status, recognized, likely_test_data, $2,
          CASE WHEN read_status = 'fresh' THEN NOW() ELSE NULL END, NULL, last_error, NOW()
        FROM jsonb_to_recordset($1::jsonb) AS row_data(
          source_tab_key text, source_kind text, spreadsheet_id text, external_tab_id text,
          current_title text, mode text, read_status text, recognized boolean,
          likely_test_data boolean, last_error text
        )
        ON CONFLICT (source_tab_key) DO UPDATE SET
          current_title = EXCLUDED.current_title,
          mode = EXCLUDED.mode,
          read_status = EXCLUDED.read_status,
          recognized = EXCLUDED.recognized,
          likely_test_data = EXCLUDED.likely_test_data,
          last_seen_scan_id = EXCLUDED.last_seen_scan_id,
          last_read_at = COALESCE(EXCLUDED.last_read_at, source_tabs.last_read_at),
          missing_since = NULL,
          last_error = EXCLUDED.last_error,
          updated_at = NOW()
      `,
      [JSON.stringify(currentTabs), scanId]
    );
  }

  const currentKeys = new Set(currentTabs.map((tab) => tab.source_tab_key));
  const removed = previous.filter((tab) => !currentKeys.has(tab.source_tab_key));
  if (removed.length) {
    await sql.query(
      `
        UPDATE source_tabs
        SET read_status = 'missing', missing_since = COALESCE(missing_since, NOW()), updated_at = NOW()
        WHERE source_tab_key = ANY($1::text[])
      `,
      [removed.map((tab) => tab.source_tab_key)]
    );
  }
  return [
    ...currentTabs
      .filter((tab) => tab.read_status === "fresh" && tab.recognized && tab.mode === "active")
      .map((tab) => ({
        sourceKind: tab.source_kind,
        spreadsheetId: tab.spreadsheet_id,
        externalTabId: tab.external_tab_id,
        sheetName: tab.current_title,
        removed: false
      })),
    ...removed
      .filter((tab) => tab.recognized && tab.mode === "active")
      .map((tab) => ({
        sourceKind: tab.source_kind,
        spreadsheetId: tab.spreadsheet_id,
        externalTabId: tab.external_tab_id,
...
function sourceInventoryTabKey(sourceKind, tab) {
  const workbook = String(tab.spreadsheetId || "").trim();
  const external = String(tab.externalTabId || "").trim();
  const title = String(tab.title || "").trim().toLowerCase().normalize("NFKC").replace(/\s+/g, " ");
  return `${sourceKind}|${workbook}|${external ? `id:${external}` : `name:${title}`}`;
}

export async function applySourceTabModes(entries = []) {
  await ensureSchema();
  const normalized = entries
    .map((item) => ({
      sourceKind: String(item?.sourceKind || "").trim().toLowerCase(),
      spreadsheetId: String(item?.spreadsheetId || "").trim(),
      externalTabId: String(item?.externalTabId || "").trim(),
      sheetName: String(item?.sheetName || "").trim(),
      mode: String(item?.mode || "active").trim().toLowerCase()
    }))
    .filter((item) => ["title", "thumbnail"].includes(item.sourceKind) && item.sheetName && ["active", "archive", "ignore"].includes(item.mode));
  if (!normalized.length) return 0;
  const sql = getSql();
  const rows = await sql.query(
    `
      WITH policies AS (
        SELECT *
        FROM jsonb_to_recordset($1::jsonb) AS policy(
          source_kind text,
          spreadsheet_id text,
          external_tab_id text,
          sheet_name text,
      ...
      SET source_tab_mode = policies.mode,
          updated_at = CASE WHEN run.source_tab_mode IS DISTINCT FROM policies.mode THEN NOW() ELSE run.updated_at END
      FROM policies
      WHERE run.source_kind = policies.source_kind
        AND (policies.spreadsheet_id = '' OR run.spreadsheet_id = policies.spreadsheet_id)
        AND (
          (policies.external_tab_id <> '' AND run.external_tab_id = policies.external_tab_id)
          OR (policies.external_tab_id = '' AND LOWER(TRIM(run.sheet_name)) = LOWER(TRIM(policies.sheet_name)))
        )
        AND run.source_tab_mode IS DISTINCT FROM policies.mode
      RETURNING run.test_run_id
    `,
    [JSON.stringify(normalized.map((item) => ({
      source_kind: item.sourceKind,
      spreadsheet_id: item.spreadsheetId,
      external_tab_id: item.externalTabId,
...
export async function listRecentResolvedFinishSignals({ days = 30 } = {}) {
  await ensureSchema();
  const sql = getSql();
  const windowDays = Math.max(1, Math.min(90, Number(days) || 30));
  const rows = await sql`
    WITH recent_finish AS (
      SELECT DISTINCT ON (fe.test_id)
        fe.test_id,
        COALESCE(NULLIF(TRIM(tr.channel), ''), NULLIF(TRIM(fe.channel), ''), 'Unknown source') AS channel,
        COALESCE(fe.occurred_at, fe.observed_at) AS finished_at
      FROM finish_events fe
      JOIN test_runs tr ON tr.test_run_id = fe.test_run_id
      WHERE fe.test_id IS NOT NULL
        AND fe.processing_status = 'matched'
        AND fe.source <> 'metadata'
        AND fe.result_evidence = 'studio_explicit'
        AND fe.result IN ('winner', 'performed_same', 'inconclusive')
        AND COALESCE(fe.occurred_at, fe.observed_at) >= NOW() - (${windowDays} * INTERVAL '1 day')
      ORDER BY fe.test_id, COALESCE(fe.occurred_at, fe.observed_at) DESC
    )
    SELECT
      recent_finish.channel,
      COUNT(*)::int AS resolved_count,
      MAX(recent_finish.finished_at) AS latest_finished_at
    FROM recent_finish
    WHERE EXISTS (
      SELECT 1
      FROM test_runs completed
      WHERE completed.test_id = recent_finish.test_id
        AND completed.status IN ('sheet_marked_done', 'result_logged', 'winner_found', 'no_clear')
    )
    OR EXISTS (
      SELECT 1
      FROM test_actions action
      JOIN test_runs action_run ON action_run.test_run_id = action.test_run_id
      WHERE action.undone_at IS NULL
        AND (action.test_id = recent_finish.test_id OR action_run.test_id = recent_finish.test_id)
    )
    GROUP BY recent_finish.channel
    ORDER BY LOWER(recent_finish.channel)
  `;
  return mergeResolvedFinishSignalChannels(rows, windowDays);
}
...
export async function listKnownYouTubeChannels() {
  ...
  const rows = await sql`
    WITH candidates AS (
      SELECT display_name, youtube_channel_id, priority, updated_at, 0 AS source_priority
      FROM channel_registry
      WHERE enabled = TRUE AND youtube_channel_id <> ''
      UNION ALL
      SELECT COALESCE(NULLIF(youtube_channel_title, ''), NULLIF(channel, ''), 'Unknown source'),
        youtube_channel_id, 1000, updated_at, 1
      FROM test_runs
    ...
    SELECT display_name AS channel, youtube_channel_id,
      display_name AS youtube_channel_title, updated_at
    FROM ranked
    WHERE row_rank = 1
    ORDER BY priority, LOWER(display_name)
  `;
  return rows.map((row) => ({
...
export async function syncChannelRegistry(records = []) {
  await ensureSchema();
  const sql = getSql();
  const byKey = new Map();
  for (const record of records) {
    const youtubeChannelId = String(record.youtubeChannelId || "").trim();
    const displayName = canonicalChannelName(record.youtubeChannelTitle || record.channel || record.sheetName) || "Unknown source";
    const normalizedName = normalizeMatchText(displayName).replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
    const channelKey = youtubeChannelId ? `id:${youtubeChannelId}` : `name:${normalizedName || "unknown"}`;
    ...
    const existing = byKey.get(channelKey);
    byKey.set(channelKey, {
      channel_key: channelKey,
      youtube_channel_id: youtubeChannelId,
      display_name: displayName,
      aliases: Array.from(new Set([...(existing?.aliases || []), ...aliases])),
      provisional: !youtubeChannelId
    });
  }
  ...
  await sql.query(
    `
      INSERT INTO channel_registry (
        channel_key, youtube_channel_id, display_name, aliases, provisional, updated_at
      )
      SELECT channel_key, youtube_channel_id, display_name, aliases, provisional, NOW()
      FROM jsonb_to_recordset($1::jsonb) AS row_data(
        channel_key text, youtube_channel_id text, display_name text, aliases jsonb, provisional boolean
      )
      ON CONFLICT (channel_key) DO UPDATE SET
        youtube_channel_id = CASE
          WHEN EXCLUDED.youtube_channel_id <> '' THEN EXCLUDED.youtube_channel_id
          ELSE channel_registry.youtube_channel_id
        END,
        display_name = CASE
          WHEN EXCLUDED.provisional = FALSE THEN EXCLUDED.display_name
          ELSE channel_registry.display_name
        END,
        aliases = (
          SELECT COALESCE(jsonb_agg(value ORDER BY value), '[]'::jsonb)
          FROM (
            SELECT DISTINCT value
            FROM jsonb_array_elements_text(channel_registry.aliases || EXCLUDED.aliases) AS alias_rows(value)
          ) unique_aliases
        ),
        provisional = channel_registry.provisional AND EXCLUDED.provisional,
        updated_at = NOW()
    `,
    [JSON.stringify(rows)]
  );
...
export async function rematchUnmatchedFinishEvents({ limit = 200, youtubeApiKey = "" } = {}) {
  ...
  const matchRuns = await listConnectorMatchRuns();
  const rows = await sql`
    SELECT event_id, test_id, video_id, channel_id, channel, source, raw_text, notification_url, notification_age, detected_outcome, payload, occurred_at, observed_at
    FROM finish_events
  ...
  for (const row of rows) {
    ...
    await sql`
      ...
      WHERE event_id = ${row.event_id}
    `;
    if (row.test_id) affectedTestIds.add(row.test_id);
    if (match.run.testId) affectedTestIds.add(match.run.testId);
    matched.push({
      eventId: row.event_id,
      testRunId: match.run.testRunId,
      matchedConfidence
  ...
  await refreshLogicalResultsFromEvidence({ sql, testIds: [...affectedTestIds] });
  await supersedeDuplicateFinishEvents();
  await cleanupUnmatchedFinishEvents();
  return matched;
}
...
export async function revalidateMatchedFinishEvents({ limit = 500, youtubeApiKey = "" } = {}) {
  ...
  const rows = await sql`
    SELECT finish_events.event_id, finish_events.video_id, finish_events.channel_id,
      ...
      finish_events.test_run_id, finish_events.test_id,
      target.source_kind AS target_source_kind,
      COALESCE(finish_events.payload->'youtubeResolution'->>'title', '') AS resolved_youtube_title,
      (
        finish_events.observed_at >= NOW() - INTERVAL '45 days'
        AND finish_events.video_id <> ''
        AND target.video_id = finish_events.video_id
        AND COALESCE(finish_events.payload->>'videoTitle', '') <> ''
      ) AS recent_video_guard
    FROM finish_events
  ...
  for (const row of rows) {
    ...
    const storedResolutionMismatch = Boolean(
      row.target_source_kind === "app_registry" &&
      row.resolved_youtube_title &&
      !isTrustedYouTubeSearchCandidate(event, {
        videoId: row.video_id,
        title: row.resolved_youtube_title,
        channelId: event.channelId,
        channel: event.channel
      })
    );
    if (storedResolutionMismatch) event = { ...event, videoId: "" };
    let match = matchFinishEventToRun(event, matchRuns);
    if (
      row.recent_video_guard &&
      !storedResolutionMismatch &&
      match.run?.videoId === row.video_id &&
      match.matchedConfidence === "video_id"
    ) {
      continue;
    }
    let youtubeResolution = {
    ...
    if ((needsYouTubeResolution(match) || storedResolutionMismatch) && youtubeApiKey && youtubeLookups < 25) {
      youtubeLookups += 1;
      const resolved = await resolveFinishEventVideo(event, { youtubeApiKey });
      if (resolved.summary?.resolved || !match.run || storedResolutionMismatch) {
        youtubeResolution = resolved;
        event = resolved.event;
        match = matchFinishEventToRun(event, matchRuns);
      }
...
export async function refreshLogicalResultsForTests(testIds = []) {
  await ensureSchema();
  const sql = getSql();
  await refreshLogicalResultsFromEvidence({ sql, testIds });
}

async function refreshLogicalResultsFromEvidence({ sql, testIds = [] }) {
  const uniqueTestIds = [...new Set(testIds.map(String).filter(Boolean))];
  for (const testId of uniqueTestIds) {
    const evidenceRows = await sql`
      SELECT result, result_evidence, result_semantics_version,
        explicit_winner_variant, youtube_applied_variant,
        inconclusive_reason, inconclusive_reason_evidence
      FROM (
        SELECT result, result_evidence, result_semantics_version,
          explicit_winner_variant, youtube_applied_variant,
          inconclusive_reason, inconclusive_reason_evidence,
          2 AS evidence_priority, updated_at
        FROM finish_events
        ...
        SELECT result, result_evidence, result_semantics_version,
          explicit_winner_variant, youtube_applied_variant,
          inconclusive_reason, inconclusive_reason_evidence,
          1 AS evidence_priority, updated_at
        FROM test_runs
        WHERE test_id = ${testId}
          AND status <> 'source_removed'
          AND result_evidence = 'sheet_explicit'
          AND result IN ('winner', 'performed_same', 'inconclusive')
      ) evidence
      ORDER BY evidence_priority DESC, updated_at DESC
      LIMIT 1
    `;
      ...
      SELECT EXISTS (
        ...
        WHERE test_id = ${testId}
          ...
          AND (
            ...
            OR status IN ('sheet_marked_done', 'result_logged', 'winner_found', 'no_clear')
            OR (
              result_evidence = 'sheet_explicit'
              AND result IN ('winner', 'performed_same', 'inconclusive')
            )
          )
      ) AS terminal
    ...
    const terminal = Boolean(terminalRows[0]?.terminal);
    await sql`
      UPDATE logical_tests
      SET result = ${evidence?.result || "unknown"},
          result_evidence = ${evidence?.result_evidence || "unknown"},
          result_semantics_version = ${evidence?.result_semantics_version || RESULT_SEMANTICS_VERSION},
          explicit_winner_variant = ${evidence?.explicit_winner_variant || ""},
          youtube_applied_variant = CASE
            WHEN ${evidence?.youtube_applied_variant || ""} <> ''
              THEN ${evidence?.youtube_applied_variant || ""}
            ELSE youtube_applied_variant
          END,
          inconclusive_reason = ${evidence?.inconclusive_reason || ""},
          inconclusive_reason_evidence = ${evidence?.inconclusive_reason_evidence || ""},
          lifecycle_status = ${terminal ? "finished" : "unknown"},
          updated_at = NOW()
      WHERE test_id = ${testId}
    `;
  }
...
export async function reconcileAppManagedRunsWithSheets() {
  ...
  const links = await sql`
    ...
    JOIN test_runs sheet
      ...
      AND sheet.source_tab_mode = 'active'
...
export async function repairLinkedRecordIdentities() {
  await ensureSchema();
  const sql = getSql();
  const repairedEvents = await sql`
    WITH repaired AS (
      UPDATE finish_events event
      SET test_id = run.test_id,
          updated_at = NOW()
      FROM test_runs run
      WHERE run.test_run_id = event.test_run_id
        AND run.test_id IS NOT NULL
        AND event.test_id IS DISTINCT FROM run.test_id
      RETURNING event.event_id
    )
    SELECT COUNT(*)::int AS count FROM repaired
  `;
  const repairedActions = await sql`
    WITH repaired AS (
      UPDATE test_actions action
      SET test_id = run.test_id
      FROM test_runs run
      WHERE run.test_run_id = action.test_run_id
        AND run.test_id IS NOT NULL
        AND action.test_id IS DISTINCT FROM run.test_id
      RETURNING action.action_id
    )
    SELECT COUNT(*)::int AS count FROM repaired
  `;
  return {
    events: Number(repairedEvents[0]?.count || 0),
    actions: Number(repairedActions[0]?.count || 0)
  };
}
...
async function reconcileManagedRunIntoTarget({
  ...
  targetRunId,
  confidence = "app_registry_to_sheet",
  driftReason = "reconciled_to_sheet"
}) {
  ...
  await sql`
    ...
    SET test_run_id = ${targetRunId},
        ...
        updated_at = NOW()
    FROM test_runs target
    WHERE target.test_run_id = ${targetRunId}
      AND event.test_run_id = ${managedRunId}
  `;
  await sql`
    UPDATE test_actions action
    SET test_run_id = ${targetRunId},
    ...
    FROM test_runs target
    WHERE target.test_run_id = ${targetRunId}
      AND action.test_run_id = ${managedRunId}
  `;
  await sql`
    INSERT INTO review_resolutions (
      resolution_id, target_type, target_id, action, actor_name, note, metadata, created_at
...
async function supersedeDuplicateFinishEvents() {
    ...
    WITH ranked AS (
      SELECT fe.event_id,
        ROW_NUMBER() OVER (
          PARTITION BY
            CASE
              WHEN fe.test_id IS NOT NULL THEN fe.test_id
              ELSE COALESCE(
                NULLIF(fe.video_id, ''),
                NULLIF(LOWER(fe.payload->>'videoTitle'), ''),
                LOWER(REGEXP_REPLACE(fe.raw_text, '\s+', ' ', 'g'))
              )
            END,
            CASE
              WHEN fe.test_id IS NULL THEN COALESCE(NULLIF(fe.channel_id, ''), LOWER(fe.channel))
              ELSE ''
            END,
            CASE WHEN fe.result <> 'unknown' THEN fe.result ELSE fe.detected_outcome END,
            CASE WHEN fe.test_id IS NULL THEN DATE_TRUNC('day', COALESCE(fe.occurred_at, fe.observed_at)) END,
            CASE WHEN fe.test_id IS NULL THEN LOWER(REGEXP_REPLACE(fe.raw_text, '\s+', ' ', 'g')) ELSE '' END
          ORDER BY
            CASE
              WHEN fe.raw_text ~* '(we updated your video to use the winner|results? with very similar performance|not enough (views|impressions)( to (determine|declare) a winner)?|(the )?test completed with no winner)[.!]?$'
                THEN 0
              WHEN fe.raw_text ~* 'we updated your video|results? with very similar performance|not enough (views|impressions)|completed with no winner'
                THEN 1
              ELSE 2
            END,
            CASE WHEN fe.notification_age <> '' THEN 0 ELSE 1 END,
            LENGTH(fe.raw_text) DESC,
            fe.observed_at DESC
        ) AS duplicate_rank
      FROM finish_events fe
      JOIN test_runs target ON target.test_run_id = fe.test_run_id
      WHERE fe.processing_status IN ('matched', 'superseded')
        AND fe.source <> 'metadata'
        AND fe.test_id IS NOT NULL
        AND target.status <> 'source_removed'
    )
    UPDATE finish_events fe
    SET processing_status = CASE
          WHEN ranked.duplicate_rank = 1 THEN 'matched'
          ELSE 'superseded'
        END,
        updated_at = NOW()
    FROM ranked
    WHERE fe.event_id = ranked.event_id
      AND fe.processing_status IS DISTINCT FROM CASE
        WHEN ranked.duplicate_rank = 1 THEN 'matched'
        ELSE 'superseded'
      END
  `;
  await sql`
    ...
    WHERE generic.processing_status = 'matched'
      ...
      AND (generic.result_evidence <> 'studio_explicit' OR generic.result = 'unknown')
      AND EXISTS (
        SELECT 1
...
async function resolveFinishEventVideo(event, { youtubeApiKey = "" } = {}) {
  ...
  try {
    ...
    const candidate = candidates.find((item) => isTrustedYouTubeSearchCandidate(searchEvent, item)) || null;
    if (!candidate) {
      return {
        event: {
          ...searchEvent,
...
async function ensureAppManagedRun({ sql, event, youtubeApiKey = "" }) {
  ...
  await sql`
    ...
    DO UPDATE SET
      ...
      video_title = CASE
        WHEN test_runs.status = 'source_removed' AND test_runs.drift_reason = 'incomplete_studio_signal'
          THEN EXCLUDED.video_title
        WHEN test_runs.video_title = '' THEN EXCLUDED.video_title
        ELSE test_runs.video_title
      END,
...
function runRow(row) {
  ...
  const finishEventStatus = finishEventSource
    ? finishEventSource === "metadata"
      ...
      : "confirmed_finished"
    : "";
  const baseQueueStatus = resolveBaseQueueStatus({
  ...
  return {
    ...
    videoId: row.video_id,
    sourceKind: row.source_kind,
    sourceTabMode: row.source_tab_mode || "active",
    spreadsheetId: row.spreadsheet_id,
    externalTabId: row.external_tab_id || "",
    sheetName: row.sheet_name,
    rowNumber: row.row_number,
    testType: row.test_type,
    channel: row.channel,
