...
export async function upsertScannedRuns({ records, scanId, onProgress }) {
  ...
  for (let index = 0; index < records.length; index += 1) {
    ...
    if (writeBatch.length >= writeBatchSize || index + 1 === records.length) {
      await sql.transaction(writeBatch);
      writeBatch = [];
...
export async function recordConnectorEvents({ events = [], actorName = "", connectorId = "", source = "studio_bell", youtubeApiKey = "" } = {}) {
  await ensureSchema();
  ...
  for (const item of events) {
    ...
    const youtubeResolution = await resolveFinishEventVideo(event, { youtubeApiKey });
    event = youtubeResolution.event;
    const match = matchFinishEventToRun(event, matchRuns);
...
export async function rematchUnmatchedFinishEvents({ limit = 200, youtubeApiKey = "" } = {}) {
  ...
  const rows = await sql`
    SELECT event_id, video_id, channel_id, channel, source, raw_text, notification_url, notification_age, detected_outcome, payload, observed_at
    FROM finish_events
  ...
  for (const row of rows) {
    ...
    let event = parseStudioNotification({
      ...
      channel: row.channel,
      notificationAge: row.notification_age || payload.notificationAge || "",
...
function activeQueueRun(run) {
  ...
  return !["sheet_marked_done", "result_logged", "winner_found", "no_clear"].includes(run.queueStatus);
}
